...
class MonthlyPayslip < ActiveRecord::Base
  ...
  def self.find_and_filter_by_department(salary_date, dept_id)
    ...
    if dept_id == 'All'
      payslips = find_all_by_salary_date(salary_date.to_date, order: 'payroll_category_id ASC', include: [:payroll_category])
      ...
      payslips = find_all_by_salary_date(salary_date.to_date, conditions: ['employee_id IN (?)', all_employees_in_dept], order: 'payroll_category_id ASC', include: [:payroll_category])
  ...
  def active_or_archived_employee
    employee = Employee.find(:first, conditions: "id=#{employee_id}")
    if employee.blank?
      ArchivedEmployee.find(:first, conditions: "former_id=#{employee_id}")
    else
      employee
    end
  end
  ...
  def self.total_employees_salary(start_date, end_date, dept_id = '')
    ...
    if dept_id.blank?
      total_monthly_payslips = find(:all, select: 'employee_id,amount,payroll_category_id,salary_date', order: 'salary_date desc', conditions: ['salary_date >= ? and salary_date <= ? and is_approved = 1', start_date.to_date, end_date.to_date], include: [:payroll_category])
    else
      active_employees_in_dept = Employee.find(:all, select: 'id', conditions: "employee_department_id = #{dept_id}")
      archived_employees_in_dept = ArchivedEmployee.find(:all, select: 'former_id', conditions: "employee_department_id = #{dept_id}")
      all_employees_in_dept = active_employees_in_dept.collect(&:id) + archived_employees_in_dept.collect { |a| a.former_id.to_i }
      total_monthly_payslips = find(:all, select: 'employee_id,amount,payroll_category_id,salary_date', order: 'salary_date desc', conditions: ['salary_date >= ? and salary_date <= ? and is_approved = 1 and employee_id IN (?)', start_date.to_date, end_date.to_date, all_employees_in_dept], include: [:payroll_category])
    end
