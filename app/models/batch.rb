...
class Batch < ActiveRecord::Base
  ...
  def elective_batch_subject(elect_group)
    Subject.find_all_by_batch_id_and_elective_group_id(id, elect_group, conditions: ['elective_group_id IS NOT NULL AND is_deleted = false'])
  end
  ...
  def allow_exam_acess(user)
    ...
    if user.employee? && user.role_symbols.include?(:subject_exam)
      flag = false if user.employee_record.subjects.all(conditions: "batch_id = '#{id}'").blank?
    end
    flag
  end
  ...
  def gpa_enabled?
    Configuration.has_gpa? and grading_type == '1'
  end

  def cwa_enabled?
    Configuration.has_cwa? and grading_type == '2'
  end
    ...
    unless grouped_exams.empty?
      ...
      unless students.empty?
        ...
        grouped_exams.each do |exam_group|
          subjects.each do |subject|
            ...
            students.each do |student|
              ...
              next if is_assigned_elective == 0
        ...
        subject_marks.each do |subject_mark|
          ...
          prev_marks = GroupedExamReport.find_by_student_id_and_subject_id_and_batch_id_and_score_type(student_id, subject_id, id, 's')
          if prev_marks.nil?
            GroupedExamReport.create(batch_id: id, student_id: student_id, marks: marks, score_type: 's', subject_id: subject_id)
          else
        ...
        exam_marks.each do |exam_mark|
          ...
          if prev_exam_score.nil?
            GroupedExamReport.create(batch_id: id, student_id: student_id, marks: tot_score, score_type: 'e', exam_group_id: exam_group.id)
          else
        ...
        exam_totals.each do |exam_total|
          ...
          prev_total_score = GroupedExamReport.find_by_student_id_and_batch_id_and_score_type(student_id, id, 'c')
  ...
  def perform
    ...
    prev_record = Configuration.find_by_config_key("job/Batch/#{job_type}")
    if prev_record.present?
      prev_record.update_attributes(config_value: Time.now)
    else
      Configuration.create(config_key: "job/Batch/#{job_type}", config_value: Time.now)
    end
  end

  def delete_student_cce_report_cache
    students.all(select: 'id, batch_id').each(&:delete_individual_cce_report_cache)
  end
