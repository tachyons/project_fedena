...
class Timetable < ActiveRecord::Base
  ...
  def self.tte_for_the_day(batch, date)
    entries = TimetableEntry.find(:all, joins: %i[timetable class_timing weekday], conditions: ['(timetables.start_date <= ? AND timetables.end_date >= ?)  AND timetable_entries.batch_id = ? AND class_timings.is_deleted = false AND weekdays.is_deleted = false', date, date, batch.id], order: 'class_timings.start_time')
    today = if entries.empty?
              []
            else
              entries.select { |a| a.day_of_week == date.wday }
            end
    today
  end
  ...
  def self.tte_for_the_weekday(batch, day)
    date = Date.today
    entries = TimetableEntry.find(:all, joins: %i[timetable class_timing weekday], conditions: ['(timetables.start_date <= ? AND timetables.end_date >= ?)  AND timetable_entries.batch_id = ? AND class_timings.is_deleted = false AND weekdays.is_deleted = false', date, date, batch.id], order: 'class_timings.start_time', include: %i[employee class_timing subject])
    today = if entries.empty?
              []
            else
              entries.select { |a| a.day_of_week == day }
            end
    today
  end
  ...
  def self.employee_tte(employee, date)
    ...
    entries += TimetableEntry.find(:all, joins: %i[timetable class_timing weekday], conditions: ['(timetables.start_date <= ? AND timetables.end_date >= ?) AND timetable_entries.subject_id in (?)  AND class_timings.is_deleted = false AND weekdays.is_deleted = false', date, date, elective_subjects], order: 'class_timings.start_time')
    today = if entries.empty?
              []
            else
              entries.select { |a| a.day_of_week == date.wday }
  ...
  def self.subject_tte(subject_id, date)
    ...
    entries = TimetableEntry.find(:all, joins: %i[timetable class_timing weekday], conditions: ['(timetables.start_date <= ? AND timetables.end_date >= ?)  AND timetable_entries.subject_id = ? AND class_timings.is_deleted = false AND weekdays.is_deleted = false', date, date, subject.id])
    today = if entries.empty?
              []
            else
              entries.select { |a| a.day_of_week == date.wday }
