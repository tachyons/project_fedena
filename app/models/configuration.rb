...
class Configuration < ActiveRecord::Base
  STUDENT_ATTENDANCE_TYPE_OPTIONS = [[t('daily_text').to_s, 'Daily'], [t('subject_wise_text').to_s, 'SubjectWise']].freeze

  NETWORK_STATES = [[t('online').to_s, 'Online'], [t('offline').to_s, 'Offline']].freeze
  LOCALES = []
  ...
  def validate
    if config_key == 'StudentAttendanceType'
      unless Configuration::STUDENT_ATTENDANCE_TYPE_OPTIONS.collect { |d| d[1] == config_value }.include?(true)
        errors.add_to_base("#{t('student_attendance_type_should_be_one')} #{STUDENT_ATTENDANCE_TYPE_OPTIONS}")
      end
    end
    if config_key == 'NetworkState'
      unless NETWORK_STATES.collect { |d| d[1] == config_value }.include?(true)
        errors.add_to_base("#{t('network_state_should_be_one')} #{NETWORK_STATES}")
      end
    end
  end
  ...
  class << self
    ...
    def save_institution_logo(upload)
      directory = "#{RAILS_ROOT}/public/uploads/image"
    ...
    def default_country
      default_country_value = find_by_config_key('DefaultCountry').config_value.to_i
