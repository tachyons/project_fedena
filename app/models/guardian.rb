...
class Guardian < ActiveRecord::Base
  ...
  def validate
    errors.add(:dob, "#{t('cant_be_a_future_date')}.") unless dob.nil? || dob <= Date.today
  end
  ...
  def create_guardian_user(student)
    user = User.new do |u|
      ...
      u.role = 'Parent'
      u.email = (email == '') || User.active.find_by_email(email) ? '' : email.to_s
    end
    update_attributes(user_id: user.id) if user.save
  end
  ...
  def immediate_contact_nil
    student = ward
    student.update_attributes(immediate_contact_id: nil) if student.present? && (student.immediate_contact_id == id)
  end
end
