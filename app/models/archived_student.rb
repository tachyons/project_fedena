...
class ArchivedStudent < ActiveRecord::Base
  ...
  def is_active_false
    self.is_active = 0 unless is_active == 0
  end
  ...
  def additional_detail(additional_field)
    StudentAdditionalDetail.find_by_additional_field_id_and_student_id(additional_field, former_id)
  end

  def has_retaken_exam(subject_id)
    retaken_exams = PreviousExamScore.find_all_by_student_id(former_id)
