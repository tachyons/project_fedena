...
class FinanceFeeCategory < ActiveRecord::Base
  ...
  validates_presence_of :batch_id, message: t('not_specified').to_s
  validates_uniqueness_of :name, scope: %i[batch_id is_deleted], if: 'is_deleted == false'
  ...
  def student_fee_balance(student, date)
    ...
    category_discounts = StudentCategoryFeeDiscount.find_all_by_finance_fee_category_id(id, joins: 'INNER JOIN students ON fee_discounts.receiver_id = students.student_category_id')
    total_discount = 0
  ...
  def self.common_active
    find(:all, conditions: ["finance_fee_categories.is_master = '1' and finance_fee_categories.is_deleted = 'false'"], joins: 'INNER JOIN batches on finance_fee_categories.batch_id = batches.id AND batches.is_active = 1 AND batches.is_deleted = 0 ', group: :name)
  end

  def is_collection_open
    collection = FinanceFeeCollection.find_all_by_fee_category_id(id, conditions: "start_date < '#{Date.today.to_date}' and due_date > '#{Date.today.to_date}'")
    collection&.reject! { |c| c.no_transaction_present }
    collection.present?
  end

  def have_common_particular?
    fee_particulars.find_all_by_student_category_id_and_admission_no(nil, nil).count > 0
  end
end
