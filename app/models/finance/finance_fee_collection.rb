...
class FinanceFeeCollection < ActiveRecord::Base
  ...
  def validate
    unless start_date.nil? || end_date.nil?
      errors.add_to_base(t('start_date_cant_be_after_end_date').to_s) if start_date > end_date
      errors.add_to_base(t('start_date_cant_be_after_due_date').to_s) if start_date > due_date
      errors.add_to_base(t('end_date_cant_be_after_due_date').to_s) if end_date > due_date
    end
  end
  ...
  def fee_transactions(student_id)
    FinanceFee.find_by_fee_collection_id_and_student_id(id, student_id)
  end
  ...
  def create_associates
    ...
    particlulars = FinanceFeeParticular.find_all_by_finance_fee_category_id(fee_category_id, conditions: 'is_deleted=0')
  ...
  def fees_particulars(student)
    FeeCollectionParticular.find_all_by_finance_fee_collection_id(id,
  ...
  def student_fee_balance(student)
    ...
    student_discounts = StudentFeeCollectionDiscount.find_all_by_finance_fee_collection_id_and_receiver_id(id, student.id)
    category_discounts = StudentCategoryFeeCollectionDiscount.find_all_by_finance_fee_collection_id_and_receiver_id(id, student.student_category_id)
    total_discount = 0
