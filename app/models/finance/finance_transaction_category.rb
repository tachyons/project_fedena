...
class FinanceTransactionCategory < ActiveRecord::Base
  ...
  def self.income_categories
    ...
    find(:all, conditions: "is_income = true AND name NOT IN (#{cat_names.join(',')}) and deleted = 0")
  end
  ...
  def total_income(start_date, end_date)
    if is_income
      finance_transactions.find(:all, conditions: ["transaction_date >= '#{start_date}' and transaction_date <= '#{end_date}' and master_transaction_id=0"]).map(&:amount).sum
    else
      0
    end
  end
  ...
  def total_expense(start_date, end_date)
    if is_income
      ...
      finance_transactions.find(:all, conditions: ["transaction_date >= '#{start_date}' and transaction_date <= '#{end_date}'"]).map(&:amount).sum
    end
  end
end
