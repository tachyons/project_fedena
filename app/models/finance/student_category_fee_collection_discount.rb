...
class StudentCategoryFeeCollectionDiscount < FeeCollectionDiscount
  ...
  validates_presence_of :receiver_id, message: t('student_category_cant_be_blank').to_s

  def total_payable(student = nil)
    payable = if student.nil?
                finance_fee_collection.fee_category.fee_particulars.active.map(&:amount).compact.flatten.sum
              else
                finance_fee_collection.fees_particulars(student).active.map(&:amount).compact.flatten.sum
              end
    payable
  end
