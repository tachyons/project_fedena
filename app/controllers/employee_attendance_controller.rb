...
class EmployeeAttendanceController < ApplicationController
  ...
  before_filter :protect_applied_leave, only: %i[own_leave_application cancel_application]
  before_filter :protect_manager_leave_application_view, only: [:leave_application]
  before_filter :protect_leave_history, only: %i[leave_history update_leave_history]
  ...
  def delete_leave_types
    ...
    if @attendance.blank?
      @leave_type.delete
      @leave_count.each(&:delete)
      flash[:notice] = t('flash3')
    else
      flash[:notice] = t('flash_msg12').to_s
    end
    redirect_to action: 'add_leave_types'
  end
  ...
  def employee_wise_leave_reset
    ...
    render :update do |page|
      flash.now[:notice] = t('flash_msg12').to_s
        ...
        params[:employee_attendance].each_pair do |emp, att|
          unless att == ''
            @employee_attendance = EmployeeAttendance.create(attendance_date: params[:date],
                                                             employees_id: emp, employee_leave_types_id: att)
          end
        end
        flash[:notice] = t('flash3')
