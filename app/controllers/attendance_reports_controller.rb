...
class AttendanceReportsController < ApplicationController
  ...
  def index
    ...
    elsif @current_user.privileges.map(&:name).include?('StudentAttendanceView')
      @batches = Batch.active
    elsif @current_user.employee?
      @batches = Batch.find_all_by_employee_id @current_user.employee_record.id
      @batches += @current_user.employee_record.subjects.collect(&:batch)
  ...
  def subject
    ...
    if @current_user.employee? && (@allow_access == true)
  ...
  def mode
    ...
    if @config.config_value == 'Daily'
      ...
      if params[:subject_id] == ''
        ...
        @subject = if params[:subject_id] == 'all_sub'
                     0
                   else
                     params[:subject_id]
                   end
    ...
    if @config.config_value == 'Daily'
      if @mode == 'Overall'
        ...
        @students.each do |student|
          ...
          unless @academic_days == 0
            @leaves[student.id]['percent'] = (@leaves[student.id]['total'].to_f / @academic_days) * 100
          end
        end
        render :update do |page|
      ...
      if @mode == 'Overall'
        ...
        if params[:subject_id] == '0'
          ...
          @batch.students.each do |s|
            @leaves[s.id]['leave'] = if @grouped[s.id].nil?
                                       0
                                     else
                                       @grouped[s.id].count
                                     end
  ...
  def report2
    ...
    if @start_date > @local_tzone_time.to_date
      ...
      @end_date = if @month == @today.month.to_s
  ...
  def student_details
    ...
    @batch = @student.batch
    @config = Configuration.find_by_config_key('StudentAttendanceType')
    @report = if @config.config_value == 'Daily'
                Attendance.find(:all, conditions: { student_id: @student.id, batch_id: @batch.id })
              else
                SubjectLeave.find(:all, conditions: { student_id: @student.id, batch_id: @batch.id })
  ...
  def filter2
    ...
    if request.post?
      if @config.config_value == 'Daily'
        ...
        @subject = Subject.find params[:filter][:subject] unless params[:filter][:subject] == '0'
        @report = if params[:filter][:subject] == '0'
                    @batch.subject_leaves.find(:all, conditions: { month_date: @start_date..@end_date })
                  else
                    SubjectLeave.find_all_by_subject_id(@subject.id, conditions: { month_date: @start_date..@end_date })
                  end
      end
    end
  end
