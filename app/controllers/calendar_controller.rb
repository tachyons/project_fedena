...
class CalendarController < ApplicationController
  ...
  def index
    ...
    if params[:new_month].nil?
      ...
      @show_month = if params[:new_month].to_i > passed_date.month
                      passed_date + 1.month
                    else
                      passed_date - 1.month
                    end
  ...
  def show_event_tooltip
    ...
    if (@user.student == true) || (@user.parent == true)
  ...
  def show_due_tooltip
    ...
    finance_due_check.select!(&:is_active_event)
    if @user.student? || @user.parent?
      finance_due_check.select! { |x| x.is_student_event(@user.student_record) } if @user.student
      finance_due_check.select! { |x| x.is_student_event(@user.parent_record) } if @user.parent
    elsif @user.employee?
      finance_due_check.select! { |x| x.is_employee_event(@user) }
    end
    @finance_due = []
    finance_due_check.each do |h|
      @finance_due.push h if h.start_date.to_date == h.end_date.to_date
    end
  end
  ...
  def build_common_events_hash(e, key, _today)
    if e.start_date.to_date == e.end_date.to_date
      @notifications[key.to_s] << e.start_date.to_date
    else
      (e.start_date.to_date..e.end_date.to_date).each do |d|
        @notifications[key.to_s] << d.to_date
      end
    end
  end

  def build_student_events_hash(h, key, batch_id, _today)
    if h.start_date.to_date == h.end_date.to_date
      student_batch_event = BatchEvent.find_by_batch_id(batch_id, conditions: "event_id = #{h.id}")
      @notifications[key.to_s] << h.start_date.to_date unless student_batch_event.nil?
    else
      (h.start_date.to_date..h.end_date.to_date).each do |d|
        student_batch_event = BatchEvent.find_by_batch_id(batch_id, conditions: "event_id = #{h.id}")
        @notifications[key.to_s] << d.to_date unless student_batch_event.nil?
      end
    end
  end

  def build_employee_events_hash(h, key, department_id, _today)
    if h.start_date.to_date == h.end_date.to_date
      ...
      @notifications[key.to_s] << h.start_date.to_date unless employee_dept_event.nil?
    else
      employee_dept_event = EmployeeDepartmentEvent.find_by_employee_department_id(department_id, conditions: "event_id = #{h.id}")
      (h.start_date.to_date..h.end_date.to_date).each do |d|
        @notifications[key.to_s] << d.to_date unless employee_dept_event.nil?
      end
    end
  end
  ...
  def load_notifications
    @events.each do |e|
      ...
      if e.is_due == true
        if e.is_active_event
          if @user.admin?
            ...
            build_common_events_hash(e, 'finance_due', @show_month) if e.is_student_event(student)
          elsif @user.employee?
            build_common_events_hash(e, 'finance_due', @show_month) if e.is_employee_event(@user)
          end
        end
      end

      if (e.is_common == false) && (e.is_holiday == false) && (e.is_exam == false) && (e.is_due == false) # not_common_event
        if @user.student?
          build_student_events_hash(e, 'student_batch_not_common', @user.student_record.batch_id, @show_month)
        end
        if @user.employee?
          build_employee_events_hash(e, 'employee_dept_not_common', @user.employee_record.employee_department_id, @show_month)
        end
      end

      if (e.is_common == false) && (e.is_holiday == true) # not_common_holiday_event
        if @user.student?
          build_student_events_hash(e, 'student_batch_not_common_holiday', @user.student_record.batch_id, @show_month)
        end
        if @user.employee?
          build_employee_events_hash(e, 'employee_dept_not_common_holiday', @user.employee_record.employee_department_id, @show_month)
        end
        if @user.admin?
          ...
          if e.start_date.to_date == e.end_date.to_date
            unless employee_dept_holiday_event.nil?
              @notifications['employee_dept_not_common_holiday'].push e.start_date.to_date
            end
          else
            (e.start_date.to_date..e.end_date.to_date).each do |d|
      ...
      if (e.is_common == false) && (e.is_holiday == false) && (e.is_exam == true) # not_common_exam_event
        unless e.origin.nil?
          if @user.student?
            ...
            if subject.elective_group_id.nil?
              ...
              if @user.student_record.students_subjects.map(&:subject_id).include?(subject.id)
                build_student_events_hash(e, 'student_batch_exam', @user.student_record.batch_id, @show_month)
              end
            end
          end
          build_common_events_hash(e, 'student_batch_exam', @show_month) if @user.employee?
          if @user.admin?
      ...
      if (e.is_common == false) && (e.is_holiday == false) && (e.is_due == false) && (e.is_exam == false) && @user.admin? # not_common_exam_due_event
