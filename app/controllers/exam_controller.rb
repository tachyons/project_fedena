...
class ExamController < ApplicationController
  ...
  def update_exam_form
    ...
    if @name == ''
      ...
      elective_subjects.each do |e|
        ...
        @elective_subjects.push e unless is_assigned.empty?
      end
    ...
    if params[:status] == 'schedule'
      ...
      Delayed::Job.enqueue(
        DelayedReminderJob.new(sender_id: current_user.id,
                               ...
                               subject: t('exam_scheduled').to_s,
                               body: "#{@exam_group.name} #{t('has_been_scheduled')}  <br/> #{t('view_calendar')}")
      )
    end
    if @exams.empty?
      @no_exam_notice = t('exam_scheduling_not_done').to_s
    else
      ...
      if sms_setting.application_sms_active && sms_setting.exam_result_schedule_sms_active
        students = @batch.students
        students.each do |s|
          guardian = s.immediate_contact
          recipients = []
          next unless s.is_sms_enabled
          ...
          sms = Delayed::Job.enqueue(SmsManager.new(@message, recipients)) unless recipients.empty?
        end
        @sms_setting_notice = t('exam_schedule_published').to_s if params[:status] == 'schedule'
        @sms_setting_notice = t('result_has_been_published').to_s if params[:status] == 'result'
      else
        @sms_setting_notice = t('exam_schedule_published_no_sms').to_s if params[:status] == 'schedule'
        @sms_setting_notice = t('exam_result_published_no_sms').to_s if params[:status] == 'result'
      end
      if params[:status] == 'result'
        ...
        Delayed::Job.enqueue(
          DelayedReminderJob.new(sender_id: current_user.id,
                                 recipient_ids: available_user_ids,
                                 subject: t('result_published').to_s,
  ...
  def generate_previous_reports
    if request.post?
      if params[:report][:batch_ids].blank?
  ...
  def generate_reports
    if request.post?
      unless !params[:report][:course_id].present? || (params[:report][:course_id] == '')
        @course = Course.find(params[:report][:course_id])
        if @course.has_batch_groups_with_active_batches
          unless !params[:report][:batch_group_id].present? || (params[:report][:batch_group_id] == '')
  ...
  def generated_report
    if params[:student].nil?
      if params[:exam_report].nil? || params[:exam_report][:exam_group_id].empty?
        flash[:notice] = t('flash2').to_s
    ...
    if params[:student].nil?
      ...
      if @student.nil?
        flash[:notice] = t('flash_student_notice').to_s
        redirect_to action: 'exam_wise_report' and return
      end
  ...
  def student_subject_rank
    if params[:rank_report][:subject_id] == ''
      ...
      unless @subject.elective_group_id.nil?
        @students.select! { |s| StudentsSubject.exists?(student_id: s.id, subject_id: @subject.id) }
      end
  ...
  def generated_report2
    ...
    if params[:exam_report][:subject_id] == ''
      flash[:notice] = t('flash4').to_s
  ...
  def student_batch_rank
    if params[:batch_rank].nil? || params[:batch_rank][:batch_id].empty?
      flash[:notice] = t('select_a_batch_to_continue').to_s
  ...
  def student_course_rank
    if params[:course_rank].nil? || (params[:course_rank][:course_id] == '')
      flash[:notice] = t('flash13').to_s
      redirect_to action: 'course_rank' and return
    else
      @course = Course.find(params[:course_rank][:course_id])
      if @course.has_batch_groups_with_active_batches && (!params[:course_rank][:batch_group_id].present? || (params[:course_rank][:batch_group_id] == ''))
        ...
        @sort_order = ''
        @sort_order = params[:sort_order] if params[:sort_order].present?
  ...
  def student_school_rank_pdf
    ...
    @sort_order = ''
    @sort_order = params[:sort_order] if params[:sort_order].present?
    @ranked_students = if @courses.empty?
                         []
                       else
                         @courses.first.find_course_rank(@batches.collect(&:id), @sort_order)
                       end
    render pdf: 'student_school_rank_pdf'
  end

  def student_attendance_rank
    if params[:attendance_rank].nil? || params[:attendance_rank][:batch_id].empty?
      flash[:notice] = t('select_a_batch_to_continue').to_s
  ...
  def select_type
    if params[:report_type].nil? || (params[:report_type] == '') || (params[:report_type] == 'overall')
      render(:update) do |page|
        page.replace_html 'subject-select', text: ''
      end
    else
      if params[:batch_id].nil? || (params[:batch_id] == '')
        render(:update) do |page|
          page.replace_html 'subject-select', text: ''
        end
      else
  ...
  def student_ranking_level_report
    if params[:ranking_level_report].nil? || (params[:ranking_level_report][:mode] == '')
      ...
      if params[:ranking_level_report][:mode] == 'batch'
        if params[:ranking_level_report][:batch_id] == ''
          ...
          if params[:ranking_level_report].nil? || (params[:ranking_level_report][:ranking_level_id] == '')
            flash[:notice] = t('flash17').to_s
            redirect_to action: 'ranking_level_report' and return
            ...
            if params[:ranking_level_report][:report_type] == 'subject'
              ...
              unless @ranking_level.subject_count.nil?
                if @ranking_level.full_course == true
        ...
        if params[:ranking_level_report][:course_id] == ''
          ...
          if @course.has_batch_groups_with_active_batches && (!params[:ranking_level_report][:batch_group_id].present? || (params[:ranking_level_report][:batch_group_id] == ''))
            flash[:notice] = t('flash14').to_s
            redirect_to action: 'ranking_level_report' and return
          elsif params[:ranking_level_report].nil? || (params[:ranking_level_report][:ranking_level_id] == '')
            flash[:notice] = t('flash17').to_s
            redirect_to action: 'ranking_level_report' and return
          else
            @ranking_level = RankingLevel.find(params[:ranking_level_report][:ranking_level_id])
            ...
            if @ranking_level.subject_count.nil?
              if @ranking_level.full_course == true
                @scores = []
                @students.each do |student|
                  total_student_score = 0
              ...
              if @ranking_level.marks_limit_type == 'upper'
                @scores.select! do |s| (
                  ...
                  (if s.student.batch.gpa_enabled?
                     (s.marks < @ranking_level.gpa unless @ranking_level.gpa.nil?)
                                       end) or (s.marks < @ranking_level.marks unless @ranking_level.marks.nil?))
                end
  ...
  def student_transcript
    if params[:transcript].nil? || (params[:transcript][:student_id] == '')
      flash[:notice] = t('flash21').to_s
      redirect_to action: 'transcript' and return
    else
      @batch = Batch.find(params[:transcript][:batch_id])
      if params[:flag].present? && (params[:flag] == '1')
      ...
      if @students.empty?
        ...
        if !params[:student_id].present? || params[:student_id].nil?
        ...
        batch_ids = BatchStudent.find_all_by_student_id(@student.id).map(&:batch_id)
        batch_ids << @batch.id
  ...
  def load_levels
    if params[:batch_id] == ''
      ...
      @ranking_levels = @course.ranking_levels.all.select { |r| (r.full_course == false) }
      render(:update) do |page|
  ...
  def student_combined_report
    if (params[:combined_report][:batch_id] == '') || (params[:combined_report][:designation_ids].blank? && params[:combined_report][:level_ids].blank?)
      flash[:notice] = t('flash22').to_s
  ...
  def student_combined_report_pdf
    @batch = Batch.find(params[:batch_id])
    @students = @batch.students
    @designations = ClassDesignation.find_all_by_id(params[:designations]) unless params[:designations].blank?
    @levels = RankingLevel.find_all_by_id(params[:levels]) unless params[:levels].blank?
  ...
  def generated_report4
    if params[:student].nil?
      if params[:exam_report].nil? || params[:exam_report][:batch_id].empty?
        flash[:notice] = t('select_a_batch_to_continue').to_s
        redirect_to action: 'grouped_exam_report' and return
      end
    else
      if params[:type].nil?
        flash[:notice] = t('invalid_parameters').to_s
        redirect_to action: 'grouped_exam_report' and return
      end
    end
    @previous_batch = 0
  ...
  def generated_report4_pdf
    ...
    if params[:student].nil?
      ...
      @subjects.select! { |sub| subject_ids.include?(sub.id) }
    else
      @student = Student.find(params[:student])
  ...
  def list_inactive_exam_groups
    if params[:batch_id] == ''
      ...
      @exam_groups.select! { |e| GroupedExam.exists?(exam_group_id: e.id, batch_id: params[:batch_id]) }
      render(:update) do |page|
  ...
  def edit_previous_marks
    @employee_subjects = []
    @employee_subjects = @current_user.employee_record.subjects.map(&:id) if @current_user.employee?
    @exam = Exam.find params[:exam_id], include: :exam_group
    @exam_group = @exam.exam_group
    @batch = @exam_group.batch
    unless @employee_subjects.include?(@exam.subject_id) || @current_user.admin? || @current_user.privileges.map(&:name).include?('ExaminationControl') || @current_user.privileges.map(&:name).include?('EnterResults')
  ...
  def update_previous_marks
      ...
      if exam_score.nil?
        ...
        if details[:marks].to_f <= @exam.maximum_marks.to_f
          if exam_score.update_attributes(details)
            if params[:student_ids]&.include?(student_id)
            ...
            flash[:warn_notice] = t('flash8').to_s
            @error = nil
          end
        else
          @error = true
    ...
    flash[:notice] = t('flash6').to_s if @error == true
    flash[:notice] = t('flash7').to_s if @error == false
    redirect_to controller: 'exam', action: 'edit_previous_marks', exam_id: @exam.id
  end

  def create_exam
    privilege = current_user.privileges.map(&:name)
    if current_user.admin || privilege.include?('ExaminationControl') || privilege.include?('EnterResults')
  ...
  def graph_for_generated_report
    ...
    bargraph.dot_size = 5
    bargraph.text = t('students_marks').to_s
    ...
    bargraph2.text = t('class_average').to_s
    ...
    x_legend = XLegend.new(t('subjects_text').to_s)
  ...
  def graph_for_previous_years_marks_overview
    ...
    x_legend = XLegend.new(t('academic_year').to_s)
    x_legend.set_style('{font-size: 14px; color: #778877}')

    y_legend = YLegend.new(t('total_marks').to_s)
    y_legend.set_style('{font-size: 14px; color: #770077}')
