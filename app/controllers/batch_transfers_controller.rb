...
class BatchTransfersController < ApplicationController
      ...
      if params[:transfer][:to].present?
        ...
        if @stu.empty?
          ...
          Subject.find_all_by_batch_id(batch.id).each do |sub|
            sub.employees_subjects.each(&:delete)
          end
        end
        ...
        @batches = Batch.active - @batch.to_a
        @batch.errors.add_to_base(t('select_a_batch_to_continue').to_s)
  ...
  def assign_all_previous_batch_subjects
    ...
    render(:update) do |page|
      ...
      page.replace_html 'msg', text: '<div class="flash-msg"><ul>' + msg + '</ul></p>' unless msg.empty?
      unless err.empty?
