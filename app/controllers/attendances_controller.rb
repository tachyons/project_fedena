...
class AttendancesController < ApplicationController
  ...
  def index
    ...
    elsif @current_user.privileges.map(&:name).include?('StudentAttendanceRegister')
      @batches = Batch.active
  ...
  def list_subject
    if params[:batch_id].present?
      ...
      if @current_user.employee? && (@allow_access == true) && !@current_user.privileges.map(&:name).include?('StudentAttendanceRegister')
  ...
  def show
    ...
    if @config.config_value == 'Daily'
      ...
      if @sub.elective_group_id.nil?
        ...
        elective_student_ids = StudentsSubject.find_all_by_subject_id(@sub.id).map(&:student_id)
  ...
  def daily_register
    ...
    @students.each do |student|
      @leaves[student.id] = Hash.new(false)
      next if attendances[student.id].nil?

      attendances[student.id].each do |attendance|
        @leaves[student.id][attendance.month_date.to_s] = attendance.id
      end
    end
    #    @dates=((@batch.end_date.to_date > @today.end_of_month) ? (@today.beginning_of_month..@today.end_of_month) : (@today.beginning_of_month..@batch.end_date.to_date))
    @dates = @batch.working_days(@today)
      ...
      if @absentee.save
        ...
        if sms_setting.application_sms_active && @student.is_sms_enabled && sms_setting.attendance_sms_active
          ...
          if @config.config_value == 'SubjectWise'
            ...
            elsif (@absentee.forenoon == true) && (@absentee.afternoon == false)
              message = "#{@student.first_name} #{@student.last_name} #{t('flash_msg7')} (forenoon) #{@absentee.month_date}"
            elsif (@absentee.afternoon == true) && (@absentee.forenoon == false)
          ...
          Delayed::Job.enqueue(SmsManager.new(message, recipients)) unless recipients.empty?
        end
  ...
  def edit
    @config = Configuration.find_by_config_key('StudentAttendanceType')
    @absentee = if @config.config_value == 'Daily'
                  Attendance.find params[:id]
                else
                  SubjectLeave.find params[:id]
                end
