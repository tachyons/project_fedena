...
class FinanceController < ApplicationController
  ...
  def donation
    ...
    if request.post? && @donation.save
      flash[:notice] = t('flash1').to_s
  ...
  def donation_edit
    ...
    if request.post? && @donation.update_attributes(params[:donation])
  ...
  def expense_edit
    ...
    if request.post? && @transaction.update_attributes(params[:transaction])
      flash[:notice] = t('flash4').to_s
      redirect_to action: :expense_list
    end
  end

  def expense_list; end

  def expense_list_update
    if params[:start_date].to_date > params[:end_date].to_date
      flash[:warn_notice] = t('flash17').to_s
      redirect_to action: 'expense_list'
    end
  ...
  def income_create
    ...
    flash[:notice] = t('flash5').to_s if @categories.empty?
    if request.post?
      @finance_transaction = FinanceTransaction.new(params[:finance_transaction])
      if @finance_transaction.save
        flash[:notice] = t('flash6').to_s
  ...
  def delete_transaction
    ...
    if income
      ...
      auto_transactions&.each(&:destroy)
    end
    @transaction.destroy
    flash[:notice] = t('flash18').to_s
    if income
      redirect_to action: 'income_list'
    else
      redirect_to action: 'expense_list'
  ...
  def categories
    ...
    @fixed_categories = @categories.select(&:is_fixed)
    @other_categories = @categories.reject(&:is_fixed)
  end
  ...
  def update_monthly_report
    ...
    @other_transaction_categories = FinanceTransaction.find(:all, params[:page], conditions: ["transaction_date >= '#{@start_date}' and transaction_date <= '#{@end_date}'and category_id NOT IN (#{@fixed_cat_ids.join(',')})"],
                                                                                 order: 'transaction_date').map(&:category).uniq
    ...
    FedenaPlugin::FINANCE_CATEGORY.each do |category|
      @category_transaction_totals[(category[:category_name]).to_s] = FinanceTransaction.total_transaction_amount(category[:category_name], @start_date, @end_date)
    end
  ...
  def employee_payslip_reject
    ...
    subject = t('payslip_rejected').to_s
  ...
  def view_monthly_payslip
    ...
    if request.post?
      ...
      unless post_data.blank?
        if post_data[:salary_date].present? && post_data[:department_id].present?
          @payslips = MonthlyPayslip.find_and_filter_by_department(post_data[:salary_date], post_data[:department_id])
        else
          flash[:notice] = t('select_salary_date').to_s
  ...
  def search_ajax
    ...
    unless params[:employee_department_id] == ''
      other_conditions += " AND employee_department_id = '#{params[:employee_department_id]}'"
    end
    unless params[:employee_category_id] == ''
      other_conditions += " AND employee_category_id = '#{params[:employee_category_id]}'"
    end
    unless params[:employee_position_id] == ''
      other_conditions += " AND employee_position_id = '#{params[:employee_position_id]}'"
    end
    unless params[:employee_grade_id] == ''
      other_conditions += " AND employee_grade_id = '#{params[:employee_grade_id]}'"
    end
      ...
      unless params[:query] == ''
        @employee = Employee.find(:all,
                         ...
                         OR employee_number LIKE ? OR (concat(first_name, \" \", last_name) LIKE ?))" + other_conditions,
                                               ...
                                               params[:query].to_s, params[:query].to_s],
  ...
  def master_fees
    ...
    unless params[:batch_id].blank?
      @master_categories = FinanceFeeCategory.find(:all, conditions: ["is_deleted = 'false' and is_master = 1 and batch_id=?", params[:batch_id]])
    end
      ...
      if @batches.nil?
        ...
        unless @batches.empty?
          @batches.each do |b|
            ...
            @error = true unless @finance_fee_category.save
          end
        end
      end
  ...
  def master_category_particulars_update
    ...
    render :update do |page|
      if params[:finance_fee_particular][:student_category_id].nil?
        params[:finance_fee_particular][:student_category_id] = ''
      end
  ...
  def fees_particulars_create
    @error = false
    unless params[:finance_fee_particular][:finance_fee_category_ids].blank?
      finance_fee_categories = FinanceFeeCategory.find_all_by_id(params[:finance_fee_particular][:finance_fee_category_ids].reject(&:empty?).map(&:to_i))
    end
    if finance_fee_categories.blank?
      flash[:notice] = t('select_fee_category')
    else
      batches = finance_fee_categories.map(&:batch)
      ...
      finance_fee_categories.each do |ffc|
        ...
        if params[:particulars][:select].to_s == 'student'
          if posted_admission_no.empty?
            @error = true
            @finance_fee_particular.errors.add(:admission_no, t('is_blank').to_s)
          else
            all_admission_no = admission_no = posted_admission_no.split(',')
            posted_params.delete 'admission_no'
            all_students = batches.map { |batch| batch.students.map(&:admission_no) }.flatten
            rejected_admission_no = admission_no.reject { |adm| all_students.include? adm }
            unless rejected_admission_no.empty?
              @error = true
              @finance_fee_particular.errors.add_to_base("#{rejected_admission_no.join(',')} #{t('does_not_belong_to_batch')} #{batches.map(&:full_name).join(',')}")
            end
            selected_admission_no = all_admission_no.select { |adm| ffc.batch.students.all.map(&:admission_no).include? adm }
      ...
      @particulars = FinanceFeeParticular.all(conditions: { is_deleted: false, finance_fee_category_id: finance_fee_categories.map(&:id) })
      if @error.blank?
  ...
  def additional_fees_create
    ...
    if params[:additional_fees][:due_date].to_date >= params[:additional_fees][:end_date].to_date
      if @additional_category.save && !params[:additional_fees][:start_date].strip.empty? && !params[:additional_fees][:due_date].strip.empty? && !params[:additional_fees][:end_date].strip.empty?
        ...
        subject = t('fees_submission_date').to_s
        ...
        if batch.empty?
          ...
          Event.create(title: t('fees_due').to_s, description: @additional_category.name, start_date: @due_date.to_datetime, end_date: @due_date.to_datetime, is_due: true, origin: @collection_date)
        else
          @students.each do |s|
            FinanceFee.create(student_id: s.id, fee_collection_id: @collection_date.id)
  ...
  def add_particulars_create
    ...
    @additional_category = FinanceFeeCategory.find(@collection_date.fee_category_id)
    @error = false
    if params[:finance_fee_particulars][:admission_no].nil?
      ...
      if @finance_fee_particulars.save
        @finance_fee_particulars_list = FeeCollectionParticular.find(:all, conditions: ["is_deleted = 'false' and finance_fee_collection_id = '#{@collection_date.id}'"])
      else
        @error = true
      end
      ...
      if params[:finance_fee_particulars][:admission_no].empty?
        @error = true
        @finance_fee_particulars = FeeCollectionParticular.new(params[:finance_fee_particulars])
        @finance_fee_particulars.valid?
        @finance_fee_particulars.errors.add(:admission_no, t('is_blank').to_s)
      else
        ...
        admission_no.each do |a|
          ...
          if s.nil?
            @error = true
            err += "#{a} #{t('does_not_exist')}<br />"
          else
            if (s.batch_id == @collection_date.batch_id) || @collection_date.batch_id.nil?
              @error = true unless @finance_fee_particulars.save
            else
              @error = true
              err += "#{a}#{t('does_not_belong_to_batch')} #{@collection_date.batch.full_name}. <br />"
            end
          end
        end
  ...
  def fee_collection_create
    ...
    if request.post?
      if params[:fee_collection].nil?
        ...
        @finance_fee_collection.errors.add_to_base(t('fees_category_cant_be_blank').to_s)
      else
        ...
        category.each do |c|
          ...
          if @finance_fee_collection.save
            ...
            new_event = Event.create(title: t('fees_due').to_s, description: params[:finance_fee_collection][:name], start_date: @finance_fee_collection.due_date.to_datetime, end_date: @finance_fee_collection.due_date.to_datetime, is_due: true, origin: @finance_fee_collection)
    ...
    render :update do |page|
      if params[:finance_fee_collection][:due_date].to_date >= params[:finance_fee_collection][:end_date].to_date
        if @finance_fee_collection.update_attributes(params[:finance_fee_collection])
          unless events.blank?
            events.update_attributes(start_date: @finance_fee_collection.due_date.to_datetime, end_date: @finance_fee_collection.due_date.to_datetime, description: params[:finance_fee_collection][:name])
          end
          ...
          @students.each do |s|
            recipient_ids << s.user.id unless s.has_paid_fees
          end
          Delayed::Job.enqueue(DelayedReminderJob.new(sender_id: @user.id,
                                                      recipient_ids: recipient_ids,
                                                      subject: subject,
  ...
  def load_fees_submission_batch
    ...
    if @fee.nil?
      ...
      @total_discount = 0
      unless @batch_discounts.nil?
        @total_discount += @batch_discounts.map { |s| s.total_payable(@student) * s.discount(@student) / 100 }.sum
      end
      unless @student_discounts.nil?
        @total_discount += @student_discounts.map { |s| s.total_payable(@student) * s.discount(@student) / 100 }.sum
      end
      unless @category_discounts.nil?
        @total_discount += @category_discounts.map { |s| s.total_payable(@student) * s.discount(@student) / 100 }.sum
      end
  ...
  def update_ajax
    ...
    unless params[:fine].nil?
      if @financefee.is_paid == true
        total_fees = params[:fine].to_f
      else
        total_fees += params[:fine].to_f
      end
    end
    if params[:fees][:fees_paid].to_f < 0
      @paid_fees = FinanceTransaction.find(:all, conditions: "FIND_IN_SET(id,\"#{@financefee.transaction_id}\")")
      @financefee.errors.add_to_base(t('flash23').to_s)
    else
      if params[:fees][:fees_paid].to_f > params[:total_fees].to_f
        @paid_fees = FinanceTransaction.find(:all, conditions: "FIND_IN_SET(id,\"#{@financefee.transaction_id}\")")
        @financefee.errors.add_to_base(t('flash19').to_s)
      else
        ...
        transaction.save
        tid = if @financefee.transaction_id.nil?
                transaction.id
              else
                @financefee.transaction_id + ",#{transaction.id}"
              end

        is_paid = params[:fees][:fees_paid].to_f == params[:total_fees].to_f
  ...
  def update_fine_ajax
    if request.post?
      ...
      if params[:fine][:fee].to_f < 0
        @financefee.errors.add_to_base(t('flash24').to_s)
      else
      ...
      unless query == ''
        @students_result = Student.find(:all,
                              ...
                              OR admission_no = ? OR (concat(first_name, \" \", last_name) LIKE ? ) ",
                                                     "#{query}%", "#{query}%", "#{query}%",
                                                     query.to_s, query.to_s],
                                        order: 'batch_id asc,first_name asc')
      end
    else
      unless query == ''
        @students_result = Student.find(:all,
  ...
  def fees_student_dates
    ...
    @dates.select! { |x| FinanceFee.exists?(fee_collection_id: x.id, student_id: @student.id) }
  end
  ...
  def fees_submission_save
    ...
    if request.post?
      if params[:fees][:fees_paid].to_f < 0
        flash[:notice] = t('flash23').to_s
      else
        if params[:fees][:fees_paid].to_f > params[:total_fees].to_f
          flash[:notice] = t('flash19').to_s
        else
          transaction = FinanceTransaction.new
          ...
          tid = if @financefee.transaction_id.nil?
                  ...
                  @financefee.transaction_id.to_s + ",#{transaction.id}"
                end
          is_paid = params[:fees][:fees_paid].to_f == params[:total_fees].to_f
          @financefee.update_attributes(transaction_id: tid, is_paid: is_paid)
          unless @financefee.transaction_id.blank?
    ...
    if query.length < 3
      ...
      unless query == ''
        @students_result = Student.find(:all,
                                        conditions: ["first_name LIKE ? OR middle_name LIKE ? OR last_name LIKE ?
                           OR admission_no = ? OR (concat(first_name, \" \", last_name) LIKE ? ) ",
                                                     "#{query}%", "#{query}%", "#{query}%", query.to_s, query.to_s],
                                        order: 'batch_id asc,first_name asc')
      end
    end
    render layout: false
  ...
  def fee_defaulters_pdf
    @batch = Batch.find(params[:batch_id])
    @date = FinanceFeeCollection.find(params[:date])
    @students = @date.students.select { |s| s.batch_id == @batch.id }
    @currency_type = Configuration.find_by_config_key('CurrencyType').config_value
  ...
  def report_compare
    ...
    FedenaPlugin::FINANCE_CATEGORY.each do |category|
      @category_transaction_totals2[(category[:category_name]).to_s] = FinanceTransaction.total_transaction_amount(category[:category_name], @start_date2, @end_date2)
    end
  ...
  def graph_for_update_monthly_report
    ...
    FedenaPlugin::FINANCE_CATEGORY.each do |category|
      ...
      x_labels << (category[:category_name]).to_s
      data << (transaction[:category_type] == 'income' ? amount : amount - (amount * 2))
  ...
  def graph_for_compare_monthly_report
    ...
    unless (donations_total <= 0) && (donations_total2 <= 0)
      x_labels << t('donations').to_s
      data << donations_total
    ...
    FedenaPlugin::FINANCE_CATEGORY.each do |category|
      ...
      next if (amount1 <= 0) && (amount2 <= 0)

      x_labels << (category[:category_name]).to_s
      data << (transaction1[:category_type] == 'income' ? amount1 : amount1 - (amount1 * 2))
      data2 << (transaction2[:category_type] == 'income' ? amount2 : amount2 - (amount2 * 2))
      largest_value = amount1 if largest_value < amount1
      largest_value = amount2 if largest_value < amount2
    end

    unless (income <= 0) && (income2 <= 0)
      x_labels << t('other_income').to_s
      data << income
      data2 << income2
      largest_value = income if largest_value < income
  ...
  def batch_wise_discount_create
    if params[:fee_collection].blank?
      @fee_discount = BatchFeeDiscount.new(params[:fee_discount])
      @fee_discount.errors.add_to_base(t('fees_category_cant_be_blank').to_s)
      @error = true
    else
      params[:fee_collection][:category_ids].each do |c|
        ...
        @error = true unless @fee_discount.save
      end
    end
  end
  ...
  def category_wise_fee_discount_create
    if params[:fee_collection].blank?
      ...
      @fee_discount.errors.add_to_base(t('batch_cant_be_blank').to_s)
      @error = true
    else
      params[:fee_collection][:category_ids].each do |c|
        @fee_category = FinanceFeeCategory.find(c)
  ...
  def student_wise_fee_discount_create
    ...
    if (params[:fee_discount][:finance_fee_category_id]).blank?
      @error = true
      @fee_discount.errors.add_to_base(t('fees_category_cant_blank').to_s)
    else
      @fee_category = FinanceFeeCategory.find(@fee_discount.finance_fee_category_id)
      if params[:students].blank?
        @error = true
        @fee_discount.errors.add_to_base(t('admission_cant_be_blank').to_s)
      else
  ...
  def show_fee_discounts
    ...
    @discount_edit = @fee_category.is_collection_open ? false : true
    render :update do |page|
  ...
  def update_fee_discount
    @fee_discount = FeeDiscount.find(params[:id])
    if @fee_discount.update_attributes(params[:fee_discount])
  ...
  def fixed_category_name
    ...
    FedenaPlugin::FINANCE_CATEGORY.each do |category|
      ...
      @plugin_cat << (category[:category_name]).to_s
    end
