...
class UserController < ApplicationController
  ...
  before_filter :login_required, except: %i[forgot_password login set_new_password reset_password first_login_change_password]
  before_filter :only_admin_allowed, only: %i[edit create index edit_privilege user_change_password delete list_user all]
  before_filter :protect_user_data, only: %i[profile user_change_password]
  ...
  def delete
    ...
    unless @user.nil?
      if @user.employee_record.nil?
        flash[:notice] = t('flash12').to_s if @user.destroy
      end
    end
  ...
  def dashboard
    @user = current_user
    @config = Configuration.available_modules
    @employee = @user.employee_record if [t('admin').to_s, t('employee_text').to_s].include?(@user.role_name)
    @student = Student.find_by_admission_no(@user.username) if @user.student?
    @student = Student.find_by_admission_no(@user.username[1..@user.username.length]) if @user.parent?
    @first_time_login = Configuration.get_config_value('FirstTimeLoginEnable')
    if  session[:user_id].present? && (@first_time_login == '1') && (@user.is_first_login != false)
  ...
  def forgot_password
    ...
    if request.post? && params[:reset_password]
  ...
  def login
    ...
    if selected_login_hook
      ...
      if request.post? && params[:user]
        ...
        authenticated_user = user if user.present? && User.authenticate?(@user.username, @user.password)
      end
    end
    if authenticated_user.present?
      ...
      flash[:notice] = t('login_error_message').to_s
    end
  end
  ...
  def first_login_change_password
    ...
    if (@setting == '1') && (@user.is_first_login != false)
      if request.post?
        if params[:user][:new_password] == params[:user][:confirm_password]
          if @user.update_attributes(password: params[:user][:confirm_password], is_first_login: false)
            flash[:notice] = t('password_update').to_s
      ...
      flash[:notice] = t('not_applicable').to_s
      redirect_to controller: 'user', action: 'dashboard'
    end
  end
  ...
  def logout
    ...
    flash[:notice] = t('logged_out').to_s
  ...
  def search_user_ajax
    if params[:query].nil? || params[:query].empty? || (params[:query] == ' ')
      @user = ''
    else
    ...
    if request.post?
      ...
      if user
        if params[:set_new_password][:new_password] === params[:set_new_password][:confirm_password]
          ...
          flash[:notice] = t('user.flash4').to_s
          redirect_to action: 'set_new_password', id: user.reset_password_code
        end
      else
