...
class EmployeeController < ApplicationController
  ...
  before_filter :protect_other_employee_data, only: %i[individual_payslip_pdf timetable timetable_pdf profile_payroll_details
                                                       view_payslip]
  before_filter :limit_employee_profile_access, only: %i[profile profile_pdf]
  ...
  def edit_category
    ...
    if request.post?
      if ((params[:category][:status] == 'false') && employees.blank?) || (params[:category][:status] == 'true')
  ...
  def delete_category
    employees = Employee.find(:all, conditions: "employee_category_id = #{params[:id]}")
    employees = ArchivedEmployee.find(:all, conditions: "employee_category_id = #{params[:id]}") if employees.empty?
    category_position = EmployeePosition.find(:all, conditions: "employee_category_id = #{params[:id]}")
    if employees.empty? && category_position.empty?
  ...
  def edit_position
    ...
    if request.post?
      if ((params[:position][:status] == 'false') && employees.blank?) || (params[:position][:status] == 'true')
  ...
  def delete_position
    employees = Employee.find(:all, conditions: "employee_position_id = #{params[:id]}")
    employees = ArchivedEmployee.find(:all, conditions: "employee_position_id = #{params[:id]}") if employees.empty?
    if employees.empty?
  ...
  def add_department
    ...
    if request.post? && @department.save
  ...
  def edit_department
    ...
    if request.post?
      if ((params[:department][:status] == 'false') && employees.blank?) || (params[:department][:status] == 'true')
  ...
  def delete_department
    employees = Employee.find(:all, conditions: "employee_department_id = #{params[:id]}")
    employees = ArchivedEmployee.find(:all, conditions: "employee_department_id = #{params[:id]}") if employees.empty?
    if employees.empty?
  ...
  def edit_grade
    ...
    if request.post?
      if ((params[:grade][:status] == 'false') && employees.blank?) || (params[:grade][:status] == 'true')
  ...
  def delete_grade
    employees = Employee.find(:all, conditions: "employee_grade_id = #{params[:id]}")
    employees = ArchivedEmployee.find(:all, conditions: "employee_grade_id = #{params[:id]}") if employees.empty?
    if employees.empty?
  ...
  def add_bank_details
    ...
    if request.post? && @bank_field.save
  ...
  def edit_bank_details
    ...
    if request.post? && @bank_details.update_attributes(params[:bank_details])
  ...
  def add_additional_details
    ...
    if request.post?
      ...
      unless @additional_details.empty?
        last_priority = @additional_details.map(&:priority).compact.max
  ...
  def change_field_priority
    ...
    prev_field = if params[:order] == 'up'
                   AdditionalField.find_by_priority(@additional_fields[position - 1])
                 else
                   AdditionalField.find_by_priority(@additional_fields[position + 1])
                 end
  ...
  def admission1
    ...
    if request.post?
      ...
      if @employee.employee_number.to_s.downcase == 'admin'
        @employee.errors.add(:employee_number, t('should_not_be_admin').to_s)
      else
        if @employee.save
  ...
  def edit1
    ...
    if request.post?
      if (params[:employee][:employee_number].downcase != 'admin') || @employee_user.admin
  ...
  def admission2
    ...
    @employee = Employee.find(params[:id])
    @selected_value = Configuration.default_country
    if request.post? && @employee.update_attributes(params[:employee])
      sms_setting = SmsSetting.new
      if sms_setting.application_sms_active && sms_setting.employee_sms_active
        recipient = [@employee.mobile_phone.to_s]
  ...
  def admission3
    ...
    redirect_to action: 'admission3_1', id: @employee.id if @bank_fields.empty?
    if request.post?
  ...
  def admission3_1
    ...
    redirect_to action: 'edit_privilege', id: @employee.employee_number if @additional_fields.empty?
    if request.post?
      @error = false
      mandatory_fields = AdditionalField.find(:all, conditions: { is_mandatory: true, status: true })
      mandatory_fields.each do |m|
        if params[:employee_additional_details][m.id.to_s.to_sym].present?
      ...
      unless @error == true
        params[:employee_additional_details].each_pair do |k, v|
          ...
          addl_info = addl_info.join(', ') if addl_field.input_type == 'has_many'
  ...
  def view_rep_manager
    @employee = Employee.find(params[:id])
    unless @employee.reporting_manager_id.nil?
      @reporting_manager = Employee.find(@employee.reporting_manager_id).first_name
    end
  ...
  def search_ajax
    ...
    if params[:query].length >= 3
      ...
      unless params[:query] == ''
        @employee = Employee.find(:all,
                                  conditions: ['(employee_number = ? )' + other_conditions, params[:query].to_s],
                                  order: 'employee_department_id asc,first_name asc', include: 'employee_department')
      end
    end
    render layout: false
      ...
      unless params[:query] == ''
        @employee = Employee.find(:all,
                         ...
                         OR employee_number = ? OR (concat(first_name, \" \", last_name) LIKE ? ))" + other_conditions,
                                               "#{params[:query]}%", "#{params[:query]}%", "#{params[:query]}%",
                                               params[:query].to_s, params[:query].to_s],
                                  order: 'employee_department_id asc,first_name asc')
      end
    else
      unless params[:query] == ''
        @employee = Employee.find(:all,
  ...
  def profile
    ...
    unless @employee.reporting_manager_id.nil?
      @reporting_manager = Employee.find(@employee.reporting_manager_id).full_name
    end
  ...
  def remove_new_paylist_category
    ...
    unless params[:id3] == ''
      @individual = IndividualPayslipCategory.find_all_by_employee_id_and_salary_date(employee, @salary_date, conditions: '')
    end
    @individual ||= []
  ...
  def create_monthly_payslip
    ...
    if request.post?
      ...
      if salary_date.to_date < @employee.joining_date.to_date
        ...
        if payslip_exists == []
          ...
          individual_payslips_generated&.each(&:delete)
  ...
  def update_monthly_payslip
    ...
    @individual_payslip_category.each do |pc|
      @individual_category_non_deductionable += pc.amount.to_i unless pc.is_deduction == true
    end

    @individual_payslip_category.each do |pc|
      @individual_category_deductionable += pc.amount.to_i unless pc.is_deduction == false
    end
    ...
    @monthly_payslips.each do |mp|
      ...
      @non_deductionable_amount += mp.amount.to_i unless category1.is_deduction == true
    end

    @monthly_payslips.each do |mp|
      category2 = PayrollCategory.find(mp.payroll_category_id)
      @deductionable_amount += mp.amount.to_i unless category2.is_deduction == false
    end
  ...
  def timetable
    @employee = Employee.find(params[:id])
    @weekday = [t('sun').to_s, t('mon').to_s, t('tue').to_s, t('wed').to_s, t('thu').to_s, t('fri').to_s, t('sat').to_s]
    ...
    @subject_timetable_entries = @employee_timetable_subjects.map(&:timetable_entries)
    @employee_subjects_ids = @employee_subjects.map(&:id)
    @weekday_timetable = {}
  ...
  def update_rejected_employee_list
    ...
    @employees.select! { |x| x.employee.employee_department_id == department_id.to_i }
  ...
  def edit_rejected_payslip
    ...
    if request.post?
      ...
      subject = t('rejected_payslip_regenerated').to_s
      body = t('payslip_has_been_generated_for').to_s + @employee.first_name + ' ' + @employee.last_name + " (#{t('employee_number')} :#{@employee.employee_number})" + " #{t('for_the_month')} #{salary_date.to_date.strftime('%B %Y')}. #{t('kindly_approve')}"
      Delayed::Job.enqueue(DelayedReminderJob.new(sender_id: @user.id,
  ...
  def one_click_payslip_generation
    ...
    finance = Configuration.find_by_config_value('Finance')
    subject = t('payslip_generated').to_s
    body = t('message_body').to_s
    salary_date = Date.parse(params[:salary_date])
    start_date = salary_date - (salary_date.day - 1).days
    end_date = start_date + 1.month
    employees = Employee.find(:all)
    if finance_manager.nil? && finance.nil?
      employees.each do |e|
        payslip_exists = MonthlyPayslip.find_all_by_employee_id(e.id,
                                                                conditions: ['salary_date >= ? and salary_date < ?', start_date, end_date])
        next unless payslip_exists == []

        salary_structure = EmployeeSalaryStructure.find_all_by_employee_id(e.id)
        next if salary_structure == []
  ...
  def one_click_payslip_revert
    if params[:one_click_payslip][:salary_date] == ''
      ...
      employees.each do |e|
        ...
        next unless payslip_record.empty?

        individual_payslip_record = IndividualPayslipCategory.find_all_by_employee_id(e.id,
                                                                                      conditions: ['salary_date >= ? and salary_date < ?', start_date, end_date])
        next if individual_payslip_record.nil?

        individual_payslip_record.each(&:destroy)
      end
  ...
  def leave_management
    ...
    @all_employee.each do |a|
      ...
      @all_employee_total_leave_count += @all_emp_app_leaves
    end
  ...
  def delete
    employee = Employee.find(params[:id])
    if employee.has_dependency
      flash[:notice] = t('flash44').to_s
