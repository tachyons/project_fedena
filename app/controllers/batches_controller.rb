...
class BatchesController < ApplicationController
  before_filter :init_data, except: %i[assign_tutor update_employees assign_employee remove_employee batches_ajax]
  ...
  def create
    ...
    if @batch.save
      ...
      unless params[:import_subjects].nil?
        ...
        if @previous_batch.blank?
          msg = nil
          flash[:no_subject_error] = t('flash7').to_s
        else
      ...
      unless params[:import_fees].nil?
        ...
        if @previous_batch.blank?
          ...
          categories.each do |c|
            ...
            particulars.reject!(&:deleted_category)
            ...
            if particulars.blank? && batch_discounts.blank? && category_discounts.blank?
              ...
              if new_category.save
                ...
                particulars.each do |p|
                  ...
                  err += "<li> #{t('particular')} #{p.name} #{t('import_failed')}.</li>" unless new_particular.save
                end
  ...
  def init_data
    @batch = Batch.find params[:id] if %w[show edit update destroy].include? action_name
