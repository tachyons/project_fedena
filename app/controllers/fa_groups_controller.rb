...
class FaGroupsController < ApplicationController
  ...
  def destroy
    @fa_group = FaGroup.find(params[:id])
    flash[:notice] = if @fa_group.update_attribute(:is_deleted, true)
                       'FA Group deleted'
                     else
                       'Unable to delete FA Group.'
                     end
  ...
  def destroy_fa_criteria
    ...
    flash[:notice] = if @fa_criteria.update_attribute(:is_deleted, true)
                       'scholastic criteria deleted'
                     else
                       'scholastic criteria cannot be deleted'
                     end
    @fa_criterias = @fa_criteria.fa_group.fa_criterias.active
    render(:update) do |page|
