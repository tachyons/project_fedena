...
module ApplicationHelper
  def get_stylesheets
    @direction = rtl? ? 'rtl/' : ''
    stylesheets ||= []
    if (controller.controller_path == 'user') && (controller.action_name == 'dashboard')
      stylesheets << @direction + '_layouts/dashboard'
    elsif (controller.controller_path == 'user') && ((controller.action_name == 'login') || (controller.action_name == 'set_new_password'))
  ...
  def main_menu
    Rails.cache.fetch("user_main_menu#{session[:user_id]}") do
  ...
  def current_school_name
    Rails.cache.fetch("current_school_name#{session[:user_id]}") do
  ...
  def generic_hook(cntrl, act)
    FedenaPlugin::ADDITIONAL_LINKS[:generic_hook].flatten.compact.each do |mod|
      next unless cntrl.to_s == mod[:source][:controller].to_s && act.to_s == mod[:source][:action].to_s
  ...
  def generic_dashboard_hook(cntrl, act)
    ...
    FedenaPlugin::ADDITIONAL_LINKS[:generic_hook].compact.flatten.each do |mod|
      ...
      next unless can_access_request? mod[:destination][:action].to_sym, mod[:destination][:controller].to_sym
  ...
  def render_generic_hook
    hooks = []
    FedenaPlugin::ADDITIONAL_LINKS[:generic_hook].compact.flatten.select do |h|
      h if h[:source][:controller] == controller_name.to_s && h[:source][:action] == action_name.to_s
    end .each do |hook|
      next unless can_access_request? hook[:destination][:action].to_sym, hook[:destination][:controller].to_sym
