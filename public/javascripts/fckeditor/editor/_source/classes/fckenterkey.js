...
var FCKEnterKey = function (
  ...
  if (tabSpaces > 0) {
    this.TabText = "";
    while (tabSpaces-- > 0) this.TabText += "\xa0";
  }
    ...
    if (!bIsStartOfBlock && !bIsEndOfBlock) {
      ...
      if (
        ...
        eNextBlock.insertBefore(
          FCKTools.GetElementDocument(eNextBlock).createTextNode("\xa0"),
          eNextBlock.firstChild
        );
      ...
      } else if (eNextBlock) eNewBlock = FCKDomTools.CloneElement(eNextBlock);

      if (!eNewBlock) eNewBlock = this.Window.document.createElement(blockTag);
      ...
      if (elementPath) {
        for (var i = 0, len = elementPath.Elements.length; i < len; i++) {
          ...
          if (
            FCKListsLib.InlineChildReqElements[element.nodeName.toLowerCase()]
          ) {
  ...
  if (oRange.StartBlockLimit == oRange.EndBlockLimit) {
    ...
    if (!bHasShift && bIsEndOfBlock && /^H[1-6]$/.test(sStartBlockTag)) {
      ...
      if (!FCKBrowserInfo.IsIE) {
        ...
        if (FCKBrowserInfo.IsSafari) FCKDomTools.ScrollIntoView(dummy, false);
        else dummy.scrollIntoView(false);
