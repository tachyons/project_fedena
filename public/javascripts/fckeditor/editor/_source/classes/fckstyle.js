...
FCKStyle.prototype = {
  ...
  RemoveFromRange: function (range, selectIt, updateRange) {
    ...
    if (range.CheckIsCollapsed()) {
      ...
      var isBoundary =
        isBoundaryRight || !FCKDomTools.GetPreviousSibling(bookmarkStart);
      ...
      for (var i = 0; i < path.Elements.length; i++) {
        ...
        if (this.CheckElementRemovable(pathElement)) {
          if (
            isBoundary &&
            !FCKDomTools.CheckIsEmptyElement(pathElement, function (el) {
                ...
                if (FCKDomTools.HasAttribute(pathElement, att)) {
                  switch (att) {
                    ...
                    case "class":
                      ...
                      if (
                        FCKDomTools.GetAttributeValue(pathElement, att) !=
        ...
        isBoundary =
          isBoundary &&
          ((isBoundaryRight && !FCKDomTools.GetNextSibling(pathElement)) ||
            (!isBoundaryRight && !FCKDomTools.GetPreviousSibling(pathElement)));
    ...
    for (var i = 1; i < pathElements.length; i++) {
      ...
      if (pathElement == path.Block || pathElement == path.BlockLimit) break;
            ...
            if (FCKDomTools.HasAttribute(currentNode, att)) {
              switch (att) {
                ...
                case "class":
                  // The 'class' element value must match (#1318).
                  if (
                    FCKDomTools.GetAttributeValue(currentNode, att) !=
                    this.GetFinalAttributeValue(att)
                  )
                    continue;
        ...
        if (FCKDomTools.HasAttribute(element, attName)) {
          ...
          if (
            ...
            (typeof attValue == "string" &&
              FCKDomTools.GetAttributeValue(element, attName) == attValue) ||
        ...
        if (FCKDomTools.HasAttribute(innerElement, att)) {
          switch (att) {
            ...
            case "class":
              // The 'class' element value must match (#1318).
              if (
                FCKDomTools.GetAttributeValue(innerElement, att) !=
                this.GetFinalAttributeValue(att)
              )
                continue;
  ...
  _CompareAttributeValues: function (attName, valueA, valueB) {
    ...
    return (
      valueA == valueB ||
      ((valueA === null || valueA === "") && (valueB === null || valueB === ""))
    );
  },
  ...
  _ApplyBlockStyle: function (range, selectIt, updateRange) {
    ...
    for (var i = 0; i < preBlocks.length - 1; i++) {
      ...
      if (
        FCKDomTools.GetNextSourceElement(preBlocks[i], true, [], [], true) !=
        preBlocks[i + 1]
      )
        continue;
    ...
    for (var i = 0; i < convertedPreBlocks.length; i++) {
      ...
      for (var j = 0; j < currentBlock.childNodes.length; j++) {
        ...
        if (
          ...
          j != 0 &&
          j != currentBlock.childNodes.length - 2 &&
          cursor.nextSibling &&
          cursor.nextSibling.nodeName.IEquals("br")
        ) {
          ...
          lastNewBlock = FCKDomTools.InsertAfterNode(
            lastNewBlock || currentBlock,
            doc.createElement(currentBlock.nodeName)
          );
          continue;
        }
  ...
  _FixBookmarkStart: function (startNode) {
    ...
    while ((startSibling = startNode.nextSibling)) {
      ...
      if (
        ...
        if (!startSibling.firstChild) FCKDomTools.RemoveNode(startSibling);
  ...
  _MergeNextSibling: function (element, attribs) {
    ...
    var hasBookmark =
      sibling && sibling.nodeType == 1 && sibling.getAttribute("_fck_bookmark");
    ...
    if (
      ...
      if (!attribs) attribs = this._CreateElementAttribsForComparison(element);
      ...
      if (this._CheckAttributesMatch(sibling, attribs)) {
        ...
        if (hasBookmark) FCKDomTools.MoveNode(element.nextSibling, element);
  ...
  _GetOverridesForComparison: function () {
    ...
    if (overridesDesc) {
      ...
      if (!FCKTools.IsArray(overridesDesc)) overridesDesc = [overridesDesc];
  ...
  _CreateElementAttribsForComparison: function (element) {
    ...
    for (var i = 0; i < element.attributes.length; i++) {
      ...
      if (att.specified) {
        attribs[att.nodeName.toLowerCase()] = FCKDomTools.GetAttributeValue(
          element,
          att
        ).toLowerCase();
        attribsCount++;
  ...
  _CheckAttributesMatch: function (element, styleAttribs) {
    ...
    for (var i = 0; i < elementAttrbs.length; i++) {
      ...
      if (att.specified) {
        ...
        if (
          styleAtt != FCKDomTools.GetAttributeValue(element, att).toLowerCase()
        )
          break;
