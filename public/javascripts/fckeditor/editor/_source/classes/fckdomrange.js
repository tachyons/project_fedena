...
FCKDomRange.prototype = {
  ...
  CheckStartOfBlock: function () {
    ...
    if (offset > 0) {
      ...
      if (container.nodeType == 3) {
        ...
        if (textValue.length != 0) return (cache.IsStartOfBlock = false);
  ...
  MoveToBookmark2: function (bookmark) {
    ...
    while (curStart.nodeType == 3 && startOffset > curStart.length) {
      if (!curStart.nextSibling || curStart.nextSibling.nodeType != 3) break;
    ...
    switch (unit) {
      ...
      case "inline_elements":
        ...
        if (this._Range.startOffset == 0) {
          ...
          if (oNode) {
            while (
              FCKListsLib.InlineNonEmptyElements[oNode.nodeName.toLowerCase()]
            ) {
