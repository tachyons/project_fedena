...
FCKTableHandler.InsertColumn = function (insertBefore) {
  ...
  if (nodes && nodes.length) oCell = nodes[insertBefore ? 0 : nodes.length - 1];
  ...
  for (var i = 0; i < oTable.rows.length; i++) {
    ...
    if (insertBefore) oRow.insertBefore(oCell, oBaseCell);
    else if (oBaseCell.nextSibling)
      oRow.insertBefore(oCell, oBaseCell.nextSibling);
    else oRow.appendChild(oCell);
  }
};
...
FCKTableHandler.DeleteColumns = function (oCell) {
  ...
  if (!oCell) {
    ...
    for (var i = aColsToDelete.length; i >= 0; i--) {
      if (aColsToDelete[i]) FCKTableHandler.DeleteColumns(aColsToDelete[i]);
    }
    return;
  }
...
FCKTableHandler._UnmarkCells = function (cells, label) {
  for (var i = 0; i < cells.length; i++) {
    if (FCKBrowserInfo.IsIE) cells[i].removeAttribute(label);
    else delete cells[i][label];
  }
};

FCKTableHandler._ReplaceCellsByMarker = function (
  tableMap,
  marker,
  substitute
) {
  ...
  for (var i = 0; i < selectionGeometry.height; i++) {
    ...
    if (rowChildNodesCount > 0)
      cellContents.appendChild(
        FCKTools.GetElementDocument(refCell).createElement("br")
      );
  }
...
FCKTableHandler.MergeDown = function () {
  ...
  if (cellContents.firstChild)
    cellContents.insertBefore(
      FCKTools.GetElementDocument(nextCell).createElement("br"),
...
FCKTableHandler.VerticalSplitCell = function () {
  ...
  var cellIndex = FCKTableHandler._GetCellIndexSpan(
    tableMap,
    currentCell.parentNode.rowIndex,
    currentCell
  );
  ...
  if (currentRowSpan > 1) {
    ...
    currentCell.parentNode.parentNode.insertBefore(
      newRow,
      currentCell.parentNode.parentNode.rows[newCellRowIndex]
    );
...
FCKTableHandler._InstallTableMap = function (tableMap, table) {
  ...
  for (var i = 0; i <= maxCol; i++) {
    for (var j = 0; j < tableMap.length; j++) {
      ...
      if (tableMap[j - 1] && tableMap[j - 1][i] == cell) cell.rowSpan++;
