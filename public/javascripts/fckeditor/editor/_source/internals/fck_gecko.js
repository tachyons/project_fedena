    ...
    var moveCursor = function () {
      ...
      if (nextTextNode) {
        ...
        if (
          ...
          if (stopNode) node.parentNode.insertBefore(marker, stopNode);
  ...
  if (FCKBrowserInfo.IsGecko) {
    ...
    this.EditorDocument.addEventListener(
      ...
      function (ev) {
        ...
        if (
          element.nodeName.IEquals("IMG", "HR", "INPUT", "TEXTAREA", "SELECT")
        ) {
...
FCK.CreateLink = function (url, noUndo) {
  ...
  if (url.length > 0) {
    ...
    for (var i = 0; i < oLinksInteractor.snapshotLength; i++) {
      ...
      // let's reset it.
      if (sTempUrl == oLink.innerHTML) oLink.innerHTML = "";
