...
var FCK = {
    ...
    while (oNode) {
      ...
      switch (oNode.nodeType) {
        ...
        case 3:
          ...
          if (oNewBlock || oNode.nodeValue.Trim().length > 0) bMoveNode = true;
      }
  ...
  GetData: function (format) {
    ...
    if (FCKBrowserInfo.IsIE) data = data.replace(FCKRegexLib.ToReplace, "$1");
  ...
  ProtectTags: function (html) {
    ...
    if (FCKBrowserInfo.IsIE)
      sTags +=
        sTags.length > 0 ? "|ABBR|XML|EMBED|OBJECT" : "ABBR|XML|EMBED|OBJECT";
  ...
  SetData: function (data, resetIsDirty) {
    ...
    if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
      ...
      if (!FCKConfig.FullPage) sHeadExtra += _FCK_GetEditorAreaStyleTags();
  ...
  Preview: function () {
    ...
    if (FCKConfig.FullPage) {
      if (FCK.TempBaseTag.length > 0) sHTML = FCK.TempBaseTag + FCK.GetXHTML();
  ...
  InsertElement: function (element) {
    ...
    if (FCKListsLib.BlockElements[elementName] != null) {
      ...
      if (FCKBrowserInfo.IsGecko) {
        if (next) next.scrollIntoView(false);
  ...
  _TabKeyHandler: function (evt) {
    ...
    if (keystrokeValue == 9 && FCK.EditMode != FCK_EDITMODE_WYSIWYG) {
      if (FCKBrowserInfo.IsIE) {
        ...
        if (range.parentElement() != FCK.EditingArea.Textarea) return true;
        range.text = "\t";
        range.select();
      } else {
        var a = [];
...
function _FCK_ProtectEvents_ReplaceEvents(eventMatch, attName) {
  return (
    " " + attName + '_fckprotectedatt="' + encodeURIComponent(eventMatch) + '"'
  );
}
