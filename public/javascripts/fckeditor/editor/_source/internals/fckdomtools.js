...
var FCKDomTools = {
  ...
  GetNextSourceElement: function (
    ...
    while (
      (currentNode = this.GetNextSourceNode(currentNode, startFromSibling))
    ) {
      // Only one "=".
      if (currentNode.nodeType == 1) {
        if (
  ...
  GetCommonParentNode: function (node1, node2, tagList) {
    ...
    while ((currentParent = commonParents.pop())) {
      if (tagMap[currentParent.nodeName.toLowerCase()]) return currentParent;
    }
    return null;
  },
  ...
  HasAttributes: function (element) {
    ...
    for (var i = 0; i < attributes.length; i++) {
      ...
      } else if (attributes[i].specified) return true;
    }
  ...
  Contains: function (mainElement, otherElement) {
    ...
    while ((otherElement = otherElement.parentNode)) {
      // Only one "="
  ...
  BreakParent: function (element, parent, reusableRange) {
    var range =
      reusableRange || new FCKDomRange(FCKTools.GetElementWindow(element));
        ...
        if (
          ...
          if (
            ...
            while (currentListItem.firstChild)
              tmp.appendChild(
                currentListItem.removeChild(currentListItem.firstChild)
              );
            currentListItem = tmp;
          }
        }
  ...
  ScrollIntoView: function (element, alignTop) {
    ...
    while ((element = element.offsetParent)) offset += element.offsetTop || 0;

    // Scroll the window to the desired position, if not already visible.
    var currentScroll = FCKTools.GetScrollPosition(window).Y;
    if (offset > 0 && offset > currentScroll) window.scrollTo(0, offset);
  },
