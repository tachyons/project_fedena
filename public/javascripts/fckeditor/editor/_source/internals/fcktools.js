...
FCKTools.FixDocumentParentWindow = function (targetWindow) {
  if (targetWindow.document) targetWindow.document.parentWindow = targetWindow;
...
FCKTools.GetElementAscensor = function (element, ascensorTagNames) {
  ...
  while (e) {
    if (lstTags.indexOf("," + e.nodeName.toUpperCase() + ",") != -1) return e;
...
FCKTools.CreateEventListener = function (func, params) {
  var f = function () {
    ...
    for (var i = 0; i < arguments.length; i++) aAllParams.push(arguments[i]);
...
FCKTools.GetDocumentPosition = function (w, node) {
  ...
  while (
    ...
    !(
      curWindow == w &&
      (curNode == w.document.body || curNode == w.document.documentElement)
    )
  ) {
