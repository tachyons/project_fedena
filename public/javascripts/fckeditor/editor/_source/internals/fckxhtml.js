  ...
  switch (htmlNode.nodeType) {
    ...
    case 1:
      ...
      if (FCKBrowserInfo.IsIE) {
        ...
        if (sNodeName.StartsWith("fck:")) sNodeName = sNodeName.Remove(0, 4);
      }
  ...
  if (bHadText)
    targetNode.appendChild(
      this.XML.createTextNode(
        textValue.replace(FCKXHtmlEntities.EntitiesRegex, FCKXHtml_GetEntity)
      )
    );
  return bHadText;
};
...
FCKXHtml.TagProcessors = {
  a: function (node, htmlNode) {
    ...
    if (htmlNode.innerHTML.Trim().length == 0 && !htmlNode.name) return false;

    var sSavedUrl = htmlNode.getAttribute("_fcksavedurl");
    if (sSavedUrl != null) FCKXHtml._AppendAttribute(node, "href", sSavedUrl);
  ...
  img: function (node, htmlNode) {
    ...
    var sSavedUrl = htmlNode.getAttribute("_fcksavedurl");
    if (sSavedUrl != null) FCKXHtml._AppendAttribute(node, "src", sSavedUrl);
  ...
  script: function (node, htmlNode) {
    ...
    node.appendChild(
      FCKXHtml.XML.createTextNode(FCKXHtml._AppendSpecialItem(htmlNode.text))
    );
