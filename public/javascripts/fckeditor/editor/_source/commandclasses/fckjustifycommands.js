...
FCKJustifyCommand.prototype = {
  Execute: function () {
    ...
    while ((block = iterator.GetNextParagraph())) {
      ...
      if (cssClassName) {
        ...
        if (currentState == FCK_TRISTATE_OFF) style.textAlign = this.AlignValue;
        else {
          style.textAlign = "";
          if (style.cssText.length == 0) block.removeAttribute("style");
        }
      }
    }
  ...
  GetState: function () {
    ...
    var currentAlign;
    if (FCKBrowserInfo.IsIE) currentAlign = firstBlock.currentStyle.textAlign;
    else
