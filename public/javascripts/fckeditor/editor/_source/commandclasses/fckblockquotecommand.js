  ...
  Execute: function () {
    ...
    if (state == FCK_TRISTATE_OFF) {
      ...
      while (paragraphs.length > 0) {
        ...
        while (block.parentNode != commonParent) block = block.parentNode;
      ...
      while ((block = iterator.GetNextParagraph())) {
        ...
        if (bqParent && bqChild) moveOutNodes.push(bqChild);
      }
      ...
      while (moveOutNodes.length > 0) {
        ...
        if (node == node.parentNode.firstChild) {
          ...
          if (!bqBlock.firstChild) bqBlock.parentNode.removeChild(bqBlock);
        ...
        while (movedNodes.length) {
          ...
          if (node.nodeName.IEquals("div")) {
            ...
            if (firstTime && needBeginBr)
              docFrag.appendChild(
                FCKTools.GetElementDocument(node).createElement("br")
              );
            var needEndBr =
              node.nextSibling &&
              !FCKListsLib.BlockBoundaries[
