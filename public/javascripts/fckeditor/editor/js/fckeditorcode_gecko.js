  ...
  ConvertToHtml: function (A) {
    if (FCKConfig.FullPage) {
      ...
      if (!FCKRegexLib.HtmlOpener.test(A))
        A =
          '<html dir="' + FCKConfig.ContentLangDirection + '">' + A + "</html>";
  ...
  StartEditor: function () {
    this.TempBaseTag =
      FCKConfig.BaseHref.length > 0
        ? '<base href="' + FCKConfig.BaseHref + '" _fcktemp="true"></base>'
        : "";
...
var FCKFocusManager = (FCK.FocusManager = {
  ...
  RemoveWindow: function (A) {
    if (FCKBrowserInfo.IsIE)
      oTarget =
        A.nodeType == 1 ? A : A.frameElement ? A.frameElement : A.document;
    ...
    var E = function () {
      ...
      if (
        !(
          ...
          (!(
            FCKListsLib.BlockElements[I] || FCKListsLib.NonEmptyBlockElements[I]
          ) &&
            B == 35)
        )
      )
      ...
      if (J) {
        ...
        while (
          ...
          !FCKListsLib.BlockElements[H.parentNode.tagName.toLowerCase()] &&
          !FCKListsLib.NonEmptyBlockElements[H.parentNode.tagName.toLowerCase()]
        )
          H = H.parentNode;
        if (
...
if (document.location.protocol == "file:") {
  ...
  FCKConfig.BasePath = document.location.pathname.substring(
    0,
    ...
    while (true) {
      ...
      if (J.indent == H) {
        ...
        if (I.nodeName.IEquals(FCKConfig.EnterMode) && I.firstChild) {
          this.TrimNode(I);
          if (
            FCKListsLib.BlockBoundaries[I.firstChild.nodeName.toLowerCase()]
          ) {
    ...
    default:
      A.appendChild(
        this.XML.createComment(
          "Element not supported - Type: " + B.nodeType + " Name: " + B.nodeName
        )
      );
      break;
  }
...
FCKStyle.prototype = {
              ...
              for (var Q in E) {
                if (FCKDomTools.HasAttribute(O, Q)) {
                  switch (Q) {
                    ...
                    case "class":
                      if (
                        FCKDomTools.GetAttributeValue(O, Q) !=
    ...
    if (C) {
      for (var i = 0; i < C.length; i++) {
        ...
        if (FCKDomTools.HasAttribute(A, D)) {
          ...
          if (
            E == null ||
            (E.test && E.test(FCKDomTools.GetAttributeValue(A, D))) ||
            (typeof E == "string" && FCKDomTools.GetAttributeValue(A, D) == E)
          )
  ...
  _ApplyBlockStyle: function (A, B, C) {
    ...
    for (var i = 0; i < H.length - 1; i++) {
      if (
        FCKDomTools.GetNextSourceElement(H[i], true, [], [], true) != H[i + 1]
      )
        continue;
    ...
    for (var i = 0; i < I.length; i++) {
      ...
      for (var j = 0; j < M.childNodes.length; j++) {
        ...
        if (
          O.nodeName.IEquals("br") &&
          j != 0 &&
          j != M.childNodes.length - 2 &&
          O.nextSibling &&
          O.nextSibling.nodeName.IEquals("br")
        ) {
      ...
      case "list_contents":
        ...
        if (
          ...
          while (
            B &&
            (B.nodeType != 1 ||
              (B != this.StartBlockLimit && !D[B.nodeName.toLowerCase()]))
          ) {
            this._Range.setStartBefore(B);
...
FCKDomRangeIterator.prototype = {
  GetNextParagraph: function () {
    ...
    if (!A) {
      ...
      if (
        ...
        this._NextNode =
          A == I
            ? null
            : FCKDomTools.GetNextSourceNode(B.EndNode, true, null, I);
        return A;
      }
    }
    if (D) {
    ...
    if (!this._NextNode)
      this._NextNode =
        C || A == I ? null : FCKDomTools.GetNextSourceNode(A, true, null, I);
    return A;
  },
};
...
FCKEnterKey.prototype.DoBackspace = function () {
  ...
  if (B.StartBlockLimit == B.EndBlockLimit && F && G) {
    ...
    if (B.CheckStartOfBlock()) {
      var I = B.StartBlock;
      var J = FCKDomTools.GetPreviousSourceElement(
        I,
        true,
...
FCKEnterKey.prototype._ExecuteEnterBlock = function (A, B) {
  ...
  if (D) {
    ...
    if (FCKBrowserInfo.IsSafari) FCKDomTools.ScrollIntoView(F || I, false);
    else if (FCKBrowserInfo.IsGeckoLike) (F || I).scrollIntoView(false);
    C.Select();
  }
  C.Release();
  return true;
  ...
  if (B.StartBlockLimit == B.EndBlockLimit) {
    ...
    if (!F && D && /^H[1-6]$/.test(E)) {
      ...
      if (!FCKBrowserInfo.IsIE) {
        ...
        if (FCKBrowserInfo.IsSafari) FCKDomTools.ScrollIntoView(I, false);
        else I.scrollIntoView(false);
  ...
  if (B && B.rangeCount == 1) {
    ...
    if (
      ...
      FCKListsLib.StyleObjectElements[
        C.startContainer.childNodes[C.startOffset].nodeName.toLowerCase()
      ]
    ) {
      A = "Control";
    }
...
FCKTableHandler.MergeDown = function () {
  ...
  if (E.firstChild)
    E.insertBefore(
      FCKTools.GetElementDocument(D).createElement("br"),
      E.firstChild
    );
  B.appendChild(E);
  this._MarkCells([D], "_Replace");
...
FCKXml.prototype = {
  LoadUrl: function (A) {
    ...
    C.open("GET", A, false);
    C.send(null);
    if (C.status == 200 || C.status == 304) B = C.responseXML;
    else if (C.status == 0 && C.readyState == 4) B = C.responseXML;
    else B = null;
    if (B) {
      try {
        var D = B.firstChild;
  ...
  while (H < G.length) {
    ...
    for (var i = 0; i < 8; i++, H++) {
      ...
      if (H >= G.length) C.style.visibility = "hidden";
        ...
        FCKTools.AddEventListenerEx(C, "click", FCKTextColorCommand_OnClick, [
          this,
          L,
        ]);
    }
    ...
    while ((E = B.GetNextParagraph())) {
      ...
      if (FCKIndentCommand._UseIndentClasses) {
        ...
        if (G < 1) E.className = H;
          ...
          E.className =
            (H.length > 0 ? H + " " : "") + FCKConfig.IndentClasses[G - 1];
      } else {
        ...
        while (R.length) {
          ...
          if (S.nodeName.IEquals("div")) {
            ...
            var Y =
              ...
              S.previousSibling &&
              !FCKListsLib.BlockBoundaries[
                S.previousSibling.nodeName.toLowerCase()
              ];
            if (W && Y)
            ...
            var Z =
              S.nextSibling &&
              !FCKListsLib.BlockBoundaries[
                S.nextSibling.nodeName.toLowerCase()
              ];
...
FCKToolbarStyleCombo.prototype.CreateItems = function (A) {
  ...
  for (var D in C) {
    var E = C[D];
    var F =
      E.GetType() == 2 ? D : FCKToolbarStyleCombo_BuildPreview(E, E.Label || D);
    var G = A.AddItem(D, F);
    G.Style = E;
  }
...
var FCKDialog = (function () {
  ...
  var G = function () {
    ...
    FCKDomTools.SetElementStyles(C, {
      width:
        Math.max(H.scrollWidth, H.clientWidth, E.scrollWidth || 0) - 1 + "px",
      height:
          ...
          if (C) {
            ...
            if (FCKBrowserInfo.IsGecko)
              D.AddItem(
                ...
                FCKLang.MergeCells,
                60,
                FCKCommands.GetCommand("TableMergeCells").GetState() == -1
              );
            else {
              D.AddItem(
                "TableMergeRight",
                FCKLang.MergeRight,
                60,
                FCKCommands.GetCommand("TableMergeRight").GetState() == -1
              );
              D.AddItem(
                "TableMergeDown",
                FCKLang.MergeDown,
                60,
                FCKCommands.GetCommand("TableMergeDown").GetState() == -1
              );
            }
            D.AddItem(
              ...
              61,
              FCKCommands.GetCommand("TableHorizontalSplitCell").GetState() ==
                -1
            );
            D.AddItem(
              ...
              61,
              FCKCommands.GetCommand("TableVerticalSplitCell").GetState() == -1
            );
            D.AddSeparator();
            D.AddItem(
              "TableCellProp",
              ...
              57,
              FCKCommands.GetCommand("TableCellProp").GetState() == -1
            );
            menu.AddSeparator();
