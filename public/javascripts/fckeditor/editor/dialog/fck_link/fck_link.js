...
function SetTarget(targetType) {
  ...
  GetE("tdPopupName").style.display = GetE("tablePopupFeatures").style.display =
    targetType == "popup" ? "" : "none";
...
function BuildOnClickPopup() {
  ...
  for (var i = 0; i < aChkFeatures.length; i++) {
    if (i > 0) sFeatures += ",";
    sFeatures +=
      aChkFeatures[i].value + "=" + (aChkFeatures[i].checked ? "yes" : "no");
  }
...
function Ok() {
  ...
  for (var i = 0; i < aLinks.length; i++) {
    ...
    SetAttribute(
      oLink,
      "tabindex",
      GetE("txtAttTabIndex").value > 0 ? GetE("txtAttTabIndex").value : null
    );
