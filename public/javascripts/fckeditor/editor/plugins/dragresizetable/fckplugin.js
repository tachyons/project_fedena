var FCKDragTableHandler = {
  ...
  _ResizeBarMouseDownListener: function (evt) {
    ...
    if (FCKDragTableHandler._LeftCell) FCKDragTableHandler._MouseMoveMode = 1;
  ...
  _ResizeBarMouseUpListener: function (evt) {
    ...
    for (var i = 0; i < tableMap.length; i++) {
      for (var j = 0; j < tableMap[i].length; j++) {
        ...
        if (tableMap[i][j - 1] != cell) cell.width = colArray[j].width;
          ...
          cell.width =
            parseInt(cell.width, 10) + parseInt(colArray[j].width, 10);
    ...
    for (var i = 0; i < processedCells.length; i++) {
      if (FCKBrowserInfo.IsIE) processedCells[i].removeAttribute("_Processed");
