...
Popup.prototype = {
  ...
  */
  initialize: function (popup, link) {
    ...
    options.trigger =
      options.trigger || (options.modal ? "click" : "mouseover");
    options.duration = this.first_value(options.duration, Popup.duration, 0.5);
    options.show_duration = this.first_value(
      options.show_duration,
      options.duration
    );
    options.hide_duration = this.first_value(
      options.hide_duration,
      options.duration
    );
    options.opacity = this.first_value(options.opacity, Popup.opacity, 0.5);
    options.show_delay = this.first_value(
      options.show_delay,
      Popup.show_delay,
      500
    );
    options.hide_delay = this.first_value(
      options.hide_delay,
      Popup.hide_delay,
      200
    );
    options.cursor_margin = this.first_value(
    ...
    if (options.closebox) {
      this.closeboxes = document.getElementsByClassName(
        options.closebox,
        this.popup
      );
  ...
  */
  ...
  show_overlay: function () {
    ...
    if (Popup.overlay_levels.length == 1) {
      ...
      // There is another modal popup at a lower level so move the overlay forward.
      Popup.overlay.style.zIndex = z;
    }
  },
  ...
  hide_overlay: function () {
    ...
    if (z) {
      // There is another modal popup at a lower level so move the overlay back.
      Popup.overlay_levels.push(z);
      Popup.overlay.style.zIndex = z;
    } else {
      // The last modal popup is being closed so hide the overlay
      // Queue the global overlay effect to ensure correct execution order.
  ...
  */
  ...
  get_popup_position: function (mouse_x, mouse_y) {
    ...
    switch (this.options.position) {
      ...
      default:
        ...
        // top property value).
        if (
          (mo = this.options.position.match(
            /^\s*([^\s,]+)\s*,\s*([^\s,]+)\s*$/
          ))
        ) {
          pos = { x: mo[1], y: mo[2] };
  ...
  get_auto_position: function (mouse_x, mouse_y) {
    ...
    var available_right =
      viewport_width - (mouse_x + this.options.cursor_margin);
    var available_left = mouse_x - this.options.cursor_margin;
    var available_top = mouse_y - this.options.cursor_margin;
    var available_bottom =
      viewport_height - (mouse_x + this.options.cursor_margin);
