...
Modalbox.Methods = {
  ...
  _init: function (options) {
    ...
    //Create the window
    this.MBwindow = Builder.node(
      "div",
      { id: "MB_window", style: "display: none" },
      [
        (this.MBframe = Builder.node("div", { id: "MB_frame" }, [
          (this.MBheader = Builder.node("div", { id: "MB_header" }, [
            (this.MBcaption = Builder.node("div", { id: "MB_caption" })),
            (this.MBclose = Builder.node(
              "a",
              { id: "MB_close", title: this.options.closeString, href: "#" },
              [Builder.build("<span>&times;</span>")]
            )),
          ])),
          (this.MBcontent = Builder.node("div", { id: "MB_content" }, [
            (this.MBloading = Builder.node(
              "div",
              { id: "MB_loading" },
  ...
  show: function (content, options) {
    ...
    if (this.MBwindow.style.display == "none") {
      ...
      // If MB already on the screen, update it
      this._update();
  ...
  _hide: function (event) {
    // Internal hide method to use inside MB class
  ...
  _appear: function () {
    ...
    if (navigator.appVersion.match(/\bMSIE\b/)) this._toggleSelects();
    this._setOverlay();
    this._setWidth();
    this._setPosition();
    if (this.options.transitions) {
    ...
    this._setWidthAndPosition = this._setWidthAndPosition.bindAsEventListener(
      this
    );
  ...
  resize: function (byWidth, byHeight, options) {
    // Change size of MB without loading content
    ...
    var newHeight =
      wHeight - hHeight + byHeight < cHeight
        ? cHeight + hHeight - wHeight
        : byHeight;
  ...
  _update: function () {
    // Updating MB in case of wizards
    Element.update(this.MBcontent, "");
    this.MBcontent.appendChild(this.MBloading);
    Element.update(this.MBloading, this.options.loadingString);
    this.currentDims = [this.MBwindow.offsetWidth, this.MBwindow.offsetHeight];
    Modalbox.resize(
      this.options.width - this.currentDims[0],
  ...
  _insertContent: function (content) {
    ...
    if (typeof content == "string") this.MBcontent.hide().update(content);
    else if (typeof this.content == "object") {
      // HTML Object is given
      ...
      this.content.getElementsBySelector("*[id]").each(function (el) {
        el.id = "MB_" + el.id;
      });
    ...
    if (this.options.height == this._options.height)
      Modalbox.resize(
        ...
        this.MBcontent.getHeight() -
          Element.getHeight(this.MBwindow) +
          Element.getHeight(this.MBheader),
        {
  ...
  _setFocus: function () {
    // Setting focus to be looped inside current MB
    if (this.focusableElements.length > 0) {
      var i = 0;
      var firstEl =
        this.focusableElements.find(function findFirst(el) {
          i++;
          return el.tabIndex == 1;
        }) || this.focusableElements.first();
      this.currFocused = i == this.focusableElements.length - 1 ? i - 1 : 0;
      firstEl.focus(); // Focus on first focusable element except close button
    } else $("MB_close").focus(); // If no focusable elements exist focus on close button
  },

  _findFocusableElements: function () {
    // Collect form elements or links from MB content
    document.body.style.cursor = "default";
    var els = this.MBcontent.getElementsBySelector(
    ...
    switch (e.keyCode) {
      ...
      case Event.KEY_END:
        ...
        if (
          ...
          !["textarea", "select"].include(node.tagName.toLowerCase())
        )
          Event.stop(e);
        else if (
          (node.tagName.toLowerCase() == "input" &&
            ["submit", "button"].include(node.type)) ||
  ...
  _preventScroll: function (event) {
    ...
    if (
      !["input", "textarea", "select", "button"].include(
  ...
  _removeElements: function () {
    ...
    if (navigator.appVersion.match(/\bMSIE\b/)) this._toggleSelects(); // Toggle back 'select' elements in IE
