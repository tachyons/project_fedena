function msgFor(num) {
  if (num < 0) return "<span class='deficit'>" + (0 - num) + " deficit</span>";
  if (num == 0) return "<span class='ok'>OK</span>";
  if (num > 0) return "<span class='remaining'>" + num + " remaining</span>";
}
...
document.observe("dom:loaded", function () {
  $$(".category-employee-options select").invoke(
    "observe",
    "change",
    recalculateAll
  );
