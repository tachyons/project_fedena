...
if (Prototype.Version < "1.6")
  alert(
    "Prototype 1.6.0 is required.  If using earlier version of prototype, please use calendar_date_select version 1.8.3"
  );
...
SelectBox.prototype = {
  initialize: function (parent_element, values, html_options, style_options) {
    this.element = $(parent_element).build(
      "select",
      html_options,
      style_options
    );
...
CalendarDateSelect.prototype = {
  initialize: function (target_element, options) {
    ...
    if (!this.options.get("embedded")) {
      ...
      Event.observe(
        document,
        "mousedown",
        (this.closeIfClickedOut_handler = this.closeIfClickedOut.bindAsEventListener(
          this
        ))
      );
      Event.observe(
        document,
        "keypress",
        (this.keyPress_handler = this.keyPress.bindAsEventListener(this))
      );
    }
  ...
  positionCalendarDiv: function () {
    ...
    if (navigator.appName == "Microsoft Internet Explorer")
      this.iframe = $(document.body).build(
        "iframe",
        { src: "javascript:false", className: "ie6_blocker" },
        {
          left: left_px,
          top: top_px,
  ...
  initCalendarDiv: function () {
    ...
    this.calendar_div = $(parent).build(
      "div",
  ...
  initCalendarGrid: function () {
    ...
    for (var cell_index = 0; cell_index < 42; cell_index++) {
      ...
      (this.calendar_day_grid[cell_index] = days_row.build(
        "td",
        {
  ...
  refreshMonthYear: function () {
    ...
    if (this.options.get("month_year") == "dropdowns") {
      ...
      if (
        ...
        (!this.year_select.setValue(y, false) ||
          e.selectedIndex <= 1 ||
  ...
  yearRange: function () {
    if (!this.flexibleYearRange())
      return $R(
        this.options.get("year_range")[0],
        this.options.get("year_range")[1]
      );

    var y = this.date.getFullYear();
    return $R(
      y - this.options.get("year_range"),
      y + this.options.get("year_range")
    );
  },
  ...
  setSelectedClass: function () {
    ...
    if (
      $R(0, 42).include(
        (days_until = this.beginning_date
          .stripTime()
          .daysDistance(this.selected_date.stripTime()))
      )
    ) {
  ...
  parseDate: function () {
    ...
    if (isNaN(this.date) && !default_time) this.date = new Date();
    else if (isNaN(this.date) && default_time)
      this.date =
        Object.prototype.toString.apply(default_time) === "[object Function]"
          ? default_time()
          : default_time;
  ...
  clearDate: function () {
    if (
      (this.target_element.disabled || this.target_element.readOnly) &&
      this.options.get("popup") != "force"
    )
      return false;
  ...
  updateSelectedDate: function (partsOrElement, via_click) {
    ...
    if (!isNaN(parts.get("minute")))
      this.selected_date.setMinutes(
        Math.floor_to_interval(
          parts.get("minute"),
  ...
  setUseTime: function (turn_on) {
    this.use_time =
      ...
      (this.options.get("time") == "mixed" ? turn_on : true); // force use_time to true if time==true && time!="mixed"
    if (this.use_time && this.selected_date) {
      // only set hour/minute if a date is already selected
      var minute = Math.floor_to_interval(
        this.selected_date.getMinutes(),
        this.options.get("minute_interval")
      );
