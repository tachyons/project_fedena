...
function changeMonth() {
  ...
  new Ajax.Request("/attendances/subject_wise_register.json", {
    parameters:
      "batch_id=" +
      this.getAttribute("batch_id") +
      "&next=" +
      this.getAttribute("next") +
      "&subject_id=" +
      $("subject_id").value,
    asynchronous: true,
    evalScripts: true,
...
function drawHeader() {
  ...
  var month = new Element("div", {
    ...
    translated[Date.parse(today).toString("MMMM")] +
      " " +
      Date.parse(today).toString("yyyy")
  );
  ...
  next.update(nextln);
  if (
    new Date(batch.start_date).clearTime() <
    Date.parse(today).moveToFirstDayOfMonth().clearTime()
  )
    header.update(prev);
  header.appendChild(month);
  if (
    new Date(batch.end_date).clearTime() >
    Date.parse(today).moveToLastDayOfMonth().clearTime()
  )
...
function makeCell(student, dt, el) {
  ...
  cellEl.id =
    ...
    d(dt) +
    "-timing-" +
    el.timetable_entry.class_timing_id;
  var ndate, tdate;
  tdate = Date.parse(date_today);
  ndate = Date.parse(dt);
  if (tdate.equals(ndate)) cellEl.addClassName("active");
  if (
    leaves[student.id][dt] == null ||
    leaves[student.id][dt][el.timetable_entry.class_timing_id] == null
  ) {
