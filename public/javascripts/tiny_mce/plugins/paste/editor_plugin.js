(function () {
  ...
  tinymce.create("tinymce.plugins.PastePlugin", {
    init: function (e, f) {
      ...
      function h(s) {
        ...
        if (e.pasteAsPlainText && (s.clipboardData || p.doc.dataTransfer)) {
          ...
          i(
            {
              content: (s.clipboardData || p.doc.dataTransfer).getData("Text"),
            },
            true
          );
          return;
      ...
      if (b(e, "paste_auto_cleanup_on_paste")) {
        if (tinymce.isOpera || /Firefox\/2/.test(navigator.userAgent)) {
          e.onKeyDown.add(function (j, k) {
            if (
              ((tinymce.isMac ? k.metaKey : k.ctrlKey) && k.keyCode == 86) ||
              (k.shiftKey && k.keyCode == 45)
            ) {
              h(k);
            }
          });
    ...
    _preProcess: function (i, f) {
      ...
      if (
        /class="?Mso|style="[^"]*\bmso-|w:WordDocument/i.test(k) ||
        f.wordContent
      ) {
        f.wordContent = true;
        e([/^\s*(&nbsp;)+/gi, /(&nbsp;|<br[^>]*>)+\s*$/gi]);
        ...
        if (
          b(l, "paste_retain_style_properties").replace(/^none$/i, "").length ==
          0
        ) {
          k = k.replace(/<\/?span[^>]*>/gi, "");
        } else {
    ...
    _postProcess: function (h, j) {
      ...
      if (
        b(f, "paste_remove_styles") ||
        (b(f, "paste_remove_styles_if_webkit") && tinymce.isWebKit)
      ) {
    ...
    _insertPlainText: function (j, x, v) {
      var t,
        ...
        m = tinymce.is,
        y = tinymce.inArray,
        g = b(j, "paste_text_linebreaktype"),
        o = b(j, "paste_text_replacements");
      function q(h) {
        c(h, function (i) {
      ...
      if (typeof v === "string" && v.length > 0) {
        if (!d) {
          d = (
            ...
            j.serializer.settings.entities
          ).split(",");
        }
        if (
        ...
        j.execCommand(
          "mceInsertRawHTML",
          false,
          v + '<span id="_plain_text_marker">&nbsp;</span>'
        );
        window.setTimeout(function () {
          var h = x.get("_plain_text_marker"),
            B,
            i,
            A,
            w;
