    ...
    init: function (c, d) {
      ...
      c.addCommand("mceFullScreen", function () {
        ...
        if (c.getParam("fullscreen_is_enabled")) {
          if (c.getParam("fullscreen_new_window")) {
            ...
            a.win.setTimeout(function () {
              ...
              tinyMCE
                .get(c.getParam("fullscreen_editor_id"))
                .setContent(c.getContent({ format: "raw" }), { format: "raw" });
              tinyMCE.remove(c);
              ...
              a.setStyle(
                a.doc.body,
                "overflow",
                c.getParam("fullscreen_overflow")
              );
              a.win.scrollTo(
                c.getParam("fullscreen_scrollx"),
                c.getParam("fullscreen_scrolly")
              );
        ...
        if (c.getParam("fullscreen_new_window")) {
          ...
          if (
            tinymce.isIE &&
            (f.fullscreen_html_overflow == "visible" ||
              f.fullscreen_html_overflow == "scroll")
          ) {
          ...
          n = a.add(a.doc.body, "div", {
            ...
            style:
              "position:" +
              (tinymce.isIE6 || (tinymce.isIE && !a.boxModel)
                ? "absolute"
                : "fixed") +
              ";top:0;left:0;width:" +
              b.w +
          ...
          f.save_onsavecallback = function () {
            c.setContent(tinyMCE.get(f.id).getContent({ format: "raw" }), {
              format: "raw",
            });
          ...
          e.fullscreenElement = new tinymce.dom.Element(
            "mce_fullscreen_container"
          );
          e.fullscreenElement.update();
          e.resizeFunc = tinymce.dom.Event.add(a.win, "resize", function () {
            var m = tinymce.DOM.getViewPort(),
              k = e.fullscreenEditor,
              j,
              l;
            j = k.dom.getSize(k.getContainer().firstChild);
            l = k.dom.getSize(
              k.getContainer().getElementsByTagName("iframe")[0]
            );
