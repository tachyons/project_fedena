  ...
  tinymce.create("tinymce.plugins.Layer", {
    ...
    _nodeChange: function (b, a, e) {
      ...
      if (!d) {
        ...
        a.setActive(
          "absolute",
          c && c.style.position.toLowerCase() == "absolute"
        );
      }
    },
    ...
    _move: function (h) {
      var b = this.editor,
        f,
        g = [],
        e = this._getParentLayer(b.selection.getNode()),
        c = -1,
        j = -1,
        a;
      a = [];
      tinymce.walk(
        ...
        function (d) {
          if (
            d.nodeType == 1 &&
            /^(absolute|relative|static)$/i.test(d.style.position)
          ) {
            a.push(d);
          }
        },
    ...
    _getParentLayer: function (a) {
      return this.editor.dom.getParent(a, function (b) {
        return (
          b.nodeType == 1 &&
          /^(absolute|relative|static)$/i.test(b.style.position)
        );
      });
    },
    ...
    _toggleAbsolute: function () {
      var a = this.editor,
        b = this._getParentLayer(a.selection.getNode());
      if (!b) {
