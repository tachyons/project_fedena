    ...
    init: function (f, g) {
      f.onBeforeRenderUI.add(function () {
        ...
        d.loadCSS(
          g +
            "/skins/" +
            (f.settings.inlinepopups_skin || "clearlooks2") +
            "/window.css"
        );
      });
    },
  ...
  tinymce.create("tinymce.InlineWindowManager:tinymce.WindowManager", {
    ...
    open: function (r, j) {
      ...
      if (r.type) {
        ...
        if (r.type) {
          k +=
            " mce" + r.type.substring(0, 1).toUpperCase() + r.type.substring(1);
        }
        r.resizable = false;
      }
      if (r.statusbar) {
      ...
      y._addAll(d.doc.body, [
        "div",
        {
          id: i,
          class:
            (q.settings.inlinepopups_skin || "clearlooks2") +
            (tinymce.isIE && window.getSelection ? " ie9" : ""),
          style: "width:100px;height:100px",
        },
        [
          "div",
          { id: i + "_wrapper", class: "mceWrapper" + k },
          [
            "div",
            { id: i + "_top", class: "mceTop" },
            ["div", { class: "mceLeft" }],
            ["div", { class: "mceCenter" }],
            ["div", { class: "mceRight" }],
            ["span", { id: i + "_title" }, r.title || ""],
          ],
          [
            "div",
            { id: i + "_middle", class: "mceMiddle" },
            ["div", { id: i + "_left", class: "mceLeft" }],
            ["span", { id: i + "_content" }],
            ["div", { id: i + "_right", class: "mceRight" }],
          ],
          [
            "div",
            { id: i + "_bottom", class: "mceBottom" },
            ["div", { class: "mceLeft" }],
            ["div", { class: "mceCenter" }],
            ["div", { class: "mceRight" }],
            ["span", { id: i + "_status" }, "Content"],
          ],
        ...
        if (u.nodeName == "A" || u.nodeName == "a") {
          if (u.className == "mceMax") {
            ...
            if (u.className == "mceMed") {
              ...
              f.iframeElement.resizeTo(
                f.oldSize.w - f.deltaWidth,
                f.oldSize.h - f.deltaHeight
              );
      ...
      if (
        y.count == 0 &&
        y.editor.getParam("dialog_type", "modal") == "modal"
      ) {
        d.add(d.doc.body, "div", {
          id: "mceModalBlocker",
          class:
            (y.editor.settings.inlinepopups_skin || "clearlooks2") +
            "_modalBlocker",
    ...
    _startDrag: function (v, G, E) {
      ...
      function D() {
        ...
        d.add(C.body, "div", {
          ...
          class:
            "mceEventBlocker " +
            (o.editor.settings.inlinepopups_skin || "clearlooks2"),
      ...
      if ((f = this.windows[i])) {
        ...
        e(d.select("div,a", i), function (k, j) {
          if (k.currentStyle.backgroundImage != "none") {
            g = new Image();
            g.src = k.currentStyle.backgroundImage.replace(
              /url\(\"(.+)\"\)/,
              "$1"
            );
          }
