...
function init() {
  ...
  if (
    /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(
  ...
  document.getElementById("filebrowsercontainer").innerHTML = getBrowserHTML(
    "filebrowser",
    "src",
    "media",
    "media"
  ...
  if (html == "") document.getElementById("linklistrow").style.display = "none";
  else document.getElementById("linklistcontainer").innerHTML = html;
...
function insertMedia() {
  ...
  if (
    ...
    fe.style.width =
      f.width.value + (f.width.value.indexOf("%") == -1 ? "px" : "");
    fe.style.height =
      f.height.value + (f.height.value.indexOf("%") == -1 ? "px" : "");
    fe.align = f.align.options[f.align.selectedIndex].value;
  } else {
    h =
      '<img src="' +
      tinyMCEPopup.getWindowArg("plugin_url") +
      '/img/trans.gif"';
...
function getType(v) {
  ...
  fo = ed
    .getParam(
      "media_types",
      "flash=swf;flv=flv;shockwave=dcr;qt=mov,qt,mpg,mp3,mp4,mpeg;shockwave=dcr;wmp=avi,wmv,wm,asf,asx,wmx,wvx;rmp=rm,ra,ram"
    )
    .split(";");
  ...
  if (v.match(/watch\?v=(.+)(.*)/)) {
    ...
    f.height.value = "350";
    f.src.value =
      "http://www.youtube.com/v/" + v.match(/v=(.*)(.*)/)[0].split("=")[1];
    return "flash";
  }
  ...
  for (i = 0; i < fo.length; i++) {
    ...
    for (x = 0; x < el.length; x++)
      if (v.indexOf("." + el[x]) != -1) return c[0];
  }
...
function switchType(v) {
  ...
  if (t == "qt" && f.src.value.toLowerCase().indexOf("rtsp://") != -1) {
    ...
    if (f.qt_qtsrc.value == "") f.qt_qtsrc.value = f.src.value;
  }
}
...
function changedType(t) {
  ...
  if (t) d.getElementById(t + "_options").style.display = "block";
}
...
function setStr(pl, p, n) {
  var f = document.forms[0],
    e = f.elements[(p != null ? p + "_" : "") + n];

  if (typeof pl[n] == "undefined") return;

  if (e.type == "text") e.value = pl[n];
  else selectByValue(f, (p != null ? p + "_" : "") + n, pl[n]);
}
...
function getStr(p, n, d) {
  ...
  if (n == "src") v = tinyMCEPopup.editor.convertURL(v, "src", null);
...
function generatePreview(c) {
  ...
  if (f.width.value != "") oldWidth = nw;

  if (f.height.value != "") oldHeight = nh;
  ...
  for (n in pl) h += n + '="' + pl[n] + '" ';

  h += "></embed>";

  // Avoid annoying warning about insecure items
  if (!tinymce.isIE || document.location.protocol != "https:") h += "</object>";
