(function (c) {
  ...
  c.create("tinymce.plugins.TablePlugin", {
    init: function (f, g) {
      ...
      function j(n) {
        var m = f.selection,
          l = f.dom.getParent(n || m.getNode(), "table");
        if (l) {
      ...
      function h() {
        ...
        f.dom.removeClass(
          f.dom.select("td.mceSelected,th.mceSelected"),
          "mceSelected"
        );
      }
      d(
      ...
      f.onInit.add(function (m) {
        ...
        if (m && m.plugins.contextmenu) {
          m.plugins.contextmenu.onContextMenu.add(function (t, r, v) {
            var w,
              u = m.selection,
              s = u.getNode() || m.getBody();
            if (
              m.dom.getParent(v, "td") ||
              m.dom.getParent(v, "th") ||
              m.dom.select("td.mceSelected,th.mceSelected").length
            ) {
              r.removeAll();
        ...
        if (!c.isIE) {
          function o() {
            var r;
            for (
              r = m.getBody().lastChild;
              r && r.nodeType == 3 && !r.nodeValue.length;
              r = r.previousSibling
            ) {}
              ...
              e.open(
                {
                  ...
                  width:
                    240 +
                    parseInt(f.getLang("table.merge_cells_delta_width", 0)),
                  height:
                    110 +
                    parseInt(f.getLang("table.merge_cells_delta_height", 0)),
                  inline: 1,
                },
                {
                  rows: n,
            ...
            e.open(
              {
                ...
                height:
                  320 + parseInt(f.getLang("table.table_delta_height", 0)),
                inline: 1,
              },
            ...
            e.open(
              {
                url: g + "/row.htm",
                width:
                  400 + parseInt(f.getLang("table.rowprops_delta_width", 0)),
                height:
                  295 + parseInt(f.getLang("table.rowprops_delta_height", 0)),
                inline: 1,
              },
              { plugin_url: g }
            );
            ...
            e.open(
              {
                url: g + "/cell.htm",
                width:
                  400 + parseInt(f.getLang("table.cellprops_delta_width", 0)),
                height:
                  295 + parseInt(f.getLang("table.cellprops_delta_height", 0)),
                inline: 1,
              },
              { plugin_url: g }
            );
