...
(function () {
  ...
  tinymce.create("tinymce.util.URI", {
    URI: function (e, g) {
      ...
      g = f.settings = g || {};
      if (
        /^(mailto|tel|news|javascript|about|data):/i.test(e) ||
        /^\s*#/.test(e)
      ) {
        f.source = e;
        return;
...
tinymce.create("static tinymce.util.JSON", {
  serialize: function (e) {
    ...
    if (b == "string") {
      ...
      return (
        '"' +
        e.replace(/([\u0080-\uFFFF\x00-\x1f\"])/g, function (g, f) {
          c = a.indexOf(f);
          if (c + 1) {
            return "\\" + a.charAt(c + 1);
          }
...
(function (m) {
  ...
  m.create("tinymce.dom.DOMUtils", {
    ...
    select: function (q, p) {
      ...
      return m.dom.Sizzle(
        q,
        o.get(p) || o.get(o.settings.root_element) || o.doc,
        []
      );
    },
    is: function (q, o) {
    ...
    setStyle: function (r, o, p) {
      ...
      return q.run(r, function (v) {
        ...
        if (
          q.pixelStyles.test(o) &&
          (m.is(p, "number") || /^[\-0-9\.]+$/.test(p))
        ) {
          p += "px";
        }
        switch (o) {
    ...
    parseStyle: function (r) {
      ...
      k(r.split(";"), function (s) {
        ...
        if (s) {
          ...
          if (v.url_converter) {
            o = o.replace(/url\([\'\"]?([^\)\'\"]+)[\'\"]?\)/g, function (
              y,
              z
            ) {
              return (
                "url(" +
                v.url_converter.call(
                  v.url_converter_scope || u,
                  u.decode(z),
                  "style",
                  null
                ) +
    ...
    isHidden: function (o) {
      o = this.get(o);
      return (
        !o || o.style.display == "none" || this.getStyle(o, "display") == "none"
      );
    },
    uniqueId: function (o) {
    ...
    setHTML: function (q, p) {
      ...
      return this.run(q, function (v) {
        ...
        if (d) {
          ...
          if (r) {
            p = p.replace(/<p ([^>]+)>|<p>/gi, '<div $1 _mce_tmp="1">');
            ...
            if (o.settings.fix_ie_paragraphs) {
              ...
              for (t = s.length - 1; t >= 0; t--) {
                ...
                if (z._mce_tmp) {
                  ...
                  z.cloneNode(false).outerHTML.replace(
                    /([a-z0-9\-_]+)=/gi,
                    function (A, x) {
                      var B;
                      if (x !== "_mce_tmp") {
    ...
    processHTML: function (r) {
      ...
      if (q.keep_values) {
        if (/<script|noscript|style/i.test(r)) {
          ...
          r = r.replace(/<script([^>]+|)>([\s\S]*?)<\/script>/gi, function (
            s,
            x,
            t
          ) {
            ...
            x = x.replace(/src=\"([^\"]+)\"?/i, function (y, z) {
              if (q.url_converter) {
                z = p.encode(
                  q.url_converter.call(
                    q.url_converter_scope || p,
                    p.decode(z),
                    "src",
                    "script"
                  )
          ...
          r = r.replace(/<style([^>]+|)>([\s\S]*?)<\/style>/gi, function (
            s,
            x,
            t
          ) {
          ...
          r = r.replace(/<noscript([^>]+|)>([\s\S]*?)<\/noscript>/g, function (
            s,
            x,
            t
          ) {
        ...
        function u(s) {
          return s.replace(h, function (y, z, x, t) {
            return (
              "<" +
              z +
              x.replace(l, function (B, A, E, D, C) {
                var F;
                A = A.toLowerCase();
                E = E || D || C || "";
                if (e[A]) {
    ...
    toHex: function (o) {
      var q = /^\s*rgb\s*?\(\s*?([0-9]+)\s*?,\s*?([0-9]+)\s*?,\s*?([0-9]+)\s*?\)\s*$/i.exec(
        o
      );
      function p(r) {
    ...
    nodeIndex: function (s, t) {
      ...
      if (s) {
        for (
          q = s.nodeType, s = s.previousSibling, r = s;
          s;
          s = s.previousSibling
        ) {
          p = s.nodeType;
        ...
        if (A.nodeType != 9) {
          ...
          if (A.nodeType == 1) {
            ...
            if (
              ...
              r[0] &&
              r[0].nodeType == 1 &&
              r[0].getAttribute("_mce_type") == "bookmark"
            ) {
    ...
    _isRes: function (o) {
      return (
        /^(top|left|bottom|right|width|height)/i.test(o) ||
        /;\s*(top|left|bottom|right|width|height)/i.test(o)
      );
    },
  });
      ...
      if (g.duplicate || g.item) {
        ...
        if (!n) {
          ...
          j.insertNode(
            m.create(
              "span",
              { _mce_type: "bookmark", id: i + "_end", style: s },
              l
            )
          );
        }
        ...
        g.insertNode(
          m.create(
            "span",
            { _mce_type: "bookmark", id: i + "_start", style: s },
            l
          )
        );
      }
  ...
  e.create("tinymce.dom.Serializer", {
    ...
    _postProcess: function (n) {
      ...
      if (n.format == "html") {
        ...
        j = j.replace(/<noscript([^>]+|)>([\s\S]*?)<\/noscript>/g, function (
          h,
          p,
          o
        ) {
      ...
      if (!B.node_filter || B.node_filter(E)) {
        switch (E.nodeType) {
          case 1:
            if (
              E.hasAttribute
                ? E.hasAttribute("_mce_bogus")
                : E.getAttribute("_mce_bogus")
            ) {
              return;
            }
            p = H = false;
            ...
            if (!H) {
              if (
                ...
                (A.invalidElementsRE && A.invalidElementsRE.test(k)) ||
                J
              ) {
                p = true;
                break;
            ...
            if (r.padd) {
              if (
                ...
                if (
                  u.hasAttribute
                    ? u.hasAttribute("_mce_bogus")
                    : u.getAttribute("_mce_bogus")
                ) {
                  y.writeText("\u00a0");
                }
              } else {
          ...
          case 3:
            if (
              B.validate_children &&
              A.elementName &&
              !A.schema.isValid(A.elementName, "#text")
            ) {
              return;
            }
    ...
    _protect: function (j) {
      ...
      f(j.patterns, function (l) {
        j.content = k(
          h(j.content).replace(l.pattern, function (n, o, m, p) {
            m = k(m);
            if (l.encode) {
              m = i._encode(m);
            }
    ...
    _setup: function () {
      ...
      if (i.invalid_elements) {
        h.invalidElementsRE = new RegExp(
          "^(" + c(i.invalid_elements.replace(/,/g, "|").toLowerCase()) + ")$"
        );
      }
...
(function (m) {
  ...
  m.create("tinymce.Editor", {
    ...
    init: function () {
      ...
      if (F.theme) {
        ...
        if (E.theme.init && F.init_theme) {
          E.theme.init(
            E,
            h.urls[F.theme] || m.documentBaseURL.replace(/\/$/, "")
          );
        }
      }
      ...
      if (F.popup_css !== false) {
        if (F.popup_css) {
          ...
          F.popup_css = E.baseURI.toAbsolute(
            "themes/" + F.theme + "/skins/" + F.skin + "/dialog.css"
          );
        }
      }
      if (F.popup_css_add) {
      ...
      E.iframeHTML =
        F.doctype + '<html><head xmlns="http://www.w3.org/1999/xhtml">';
      ...
      if (A.verify_css_classes) {
        z.serializer.attribValueFilter = function (D, B) {
          ...
          if (D == "class") {
            ...
            return !z.classesRE ||
              /(\bmceItem\w+\b|\bmceTemp\w+\b)/g.test(B) ||
              z.classesRE.test(B)
              ? B
              : "";
          }
    ...
    _addEvents: function () {
      ...
      q.bind(
        A.content_editable ? z.getBody() : a ? z.getDoc() : z.getWin(),
        "focus",
        function (s) {
          z.focus(true);
        }
      ...
      if (m.isWebKit) {
        z.onClick.add(function (s, t) {
          ...
          if (
            t.nodeName == "IMG" ||
            (t.nodeName == "A" && q.hasClass(t, "mceItemAnchor"))
          ) {
  ...
  c.EditorCommands = function (n) {
    ...
    t({
          ...
          d(p.getSelectedBlocks(), function (z) {
            if (y == "outdent") {
              ...
              l.setStyle(
                z,
                "paddingLeft",
                parseInt(z.style.paddingLeft || 0) + v + u
              );
            }
          });
        } else {
      ...
      mceReplaceContent: function (x, v, u) {
        p.setContent(
          u.replace(/\{\$selection\}/g, p.getContent({ format: "text" }))
        );
      },
    ...
    t(
      {
        ...
        Outdent: function () {
          ...
          if (k.inline_styles) {
            if (
              (u = l.getParent(p.getStart(), l.isBlock)) &&
              parseInt(u.style.paddingLeft) > 0
            ) {
              return a;
            }
            if (
              (u = l.getParent(p.getEnd(), l.isBlock)) &&
              parseInt(u.style.paddingLeft) > 0
            ) {
              return a;
            }
          ...
          return (
            ...
            m("InsertOrderedList") ||
            (!k.inline_styles && !!l.getParent(p.getNode(), "BLOCKQUOTE"))
          );
        },
  ...
  m.create("tinymce.ForceBlocks", {
      ...
      if (m.isWebKit) {
        ...
        n.onKeyPress.add(function (s, t) {
          if (
            ...
            (t.shiftKey ||
              (q.force_br_newlines &&
                !u.getParent(p.getNode(), "h1,h2,h3,h4,h5,h6,ol,ul")))
          ) {
            r(s);
            k.cancel(t);
          }
    ...
    forceRoots: function (x, I) {
      ...
      for (E = u.length - 1; E >= 0; E--) {
        ...
        if (
          ...
          (!z.dom.isBlock(L) &&
            L.nodeType !== 8 &&
            !/^(script|mce:script|style|mce:style)$/i.test(L.nodeName))
        ) {
          if (!q) {
              ...
              if (N == -2 && B) {
                if (!c || B.setStart) {
                  if (
                    ...
                    if (
                      x.dom.getParent(B.startContainer, function (n) {
                        return n === M;
                      })
                    ) {
                      G = B.startOffset;
    ...
    insertPara: function (S) {
      ...
      if (
        v &&
        (v.nodeName == "CAPTION" ||
          /absolute|relative|fixed/gi.test(O.getStyle(v, "position", 1)))
      ) {
        o = X.element;
        v = null;
      }
      ...
      if (
        /(TD|TABLE|TH|CAPTION)/.test(o) ||
        (v && o == "DIV" && /left|right/gi.test(O.getStyle(v, "float", 1)))
      ) {
        o = X.element;
        v = A = null;
      }
      ...
      do {
        if (
          ...
          K.nodeType == 9 ||
          G.dom.isBlock(K) ||
          /(TD|TABLE|TH|CAPTION)/.test(K.nodeName)
        ) {
          break;
        }
        q = K;
      ...
      function B(r) {
        return (
          T.createTreeWalker(r, NodeFilter.SHOW_TEXT, null, h).nextNode() || r
        );
      }
      I = T.createRange();
        ...
        if (ad) {
          if (ad.length === o) {
            ...
            for (ae = 0; ae < ad.length; ae++) {
              if (
                aj === "attributes" ? c.getAttrib(ag, ad[ae]) : J(ag, ad[ae])
              ) {
                return ai;
              }
            }
      ...
      function aa(ad, ac) {
        ...
        if (
          !ae(
            ...
            c.parseStyle(c.getAttrib(ac, "style"))
          )
        ) {
          return Q;
        }
        ...
        if (!N.isListening && ab()) {
          ...
          M("onKeyDown,onKeyUp,onKeyPress,onMouseUp".split(","), function (ad) {
            T[ad].addToTop(function (ae, af) {
              if (
                ab() &&
                !a.dom.RangeUtils.compareRanges(N.lastRng, p.getRng())
              ) {
