  ...
  e.create("tinymce.themes.AdvancedTheme", {
    ...
    init: function (j, k) {
      ...
      j.onInit.add(function () {
        ...
        if (j.settings.content_css !== false) {
          j.dom.loadCSS(
            j.baseURI.toAbsolute(
              k + "/skins/" + j.settings.skin + "/content.css"
            )
          );
        }
      });
      j.onSetProgressState.add(function (q, o, r) {
        ...
        if (o) {
          l.progressTimer = setTimeout(function () {
            s = q.getContainer();
            s = s.insertBefore(
              d.create("DIV", { style: "position:relative" }),
              s.firstChild
            );
            p = d.get(q.id + "_tbl");
            d.add(s, "div", {
      ...
      d.loadCSS(
        m.editor_css
          ? j.documentBaseURI.toAbsolute(m.editor_css)
          : k + "/skins/" + j.settings.skin + "/ui.css"
      );
      if (m.skin_variant) {
        d.loadCSS(
          k + "/skins/" + j.settings.skin + "/ui_" + m.skin_variant + ".css"
        );
      }
    },
    ...
    renderUI: function (k) {
      ...
      m = j = d.create("span", {
        ...
        class:
          ...
          "Skin" +
          (w.skin_variant
            ? " " + r.settings.skin + "Skin" + v._ufirst(w.skin_variant)
            : ""),
      });
      if (!d.boxModel) {
    ...
    _rowLayout: function (w, m, k) {
      ...
      f(c(w.theme_advanced_containers || ""), function (s, o) {
        ...
        switch (n.toLowerCase()) {
          ...
          default:
            q = (
              w["theme_advanced_container_" + s + "_align"] || x
            ).toLowerCase();
            q = "mce" + v._ufirst(q);
            l = d.add(d.add(m, "tr"), "td", {
              class:
                "mceToolbar " +
                  (w["theme_advanced_container_" + s + "_class"] || u) +
                  " " +
                  q || x,
            });
    ...
    _addToolbars: function (w, k) {
      ...
      if (!r.getParam("accessibility_focus")) {
        q.push(
          d.createHTML(
            "a",
            { href: "#", onfocus: "tinyMCE.get('" + r.id + "').focus();" },
            "<!-- IE -->"
          )
        );
      }
    ...
    _nodeChanged: function (r, z, l, x, j) {
      ...
      if (D.theme_advanced_path && D.theme_advanced_statusbar_location) {
        i =
          d.get(r.id + "_path") ||
          d.add(r.id + "_path_row", "span", { id: r.id + "_path" });
        d.setHTML(i, "");
        o(function (E) {
          ...
          if (
            E.nodeType != 1 ||
            E.nodeName === "BR" ||
            d.hasClass(E, "mceItemHidden") ||
            d.hasClass(E, "mceItemRemoved")
          ) {
            return;
          }
      ...
      i.windowManager.open(
        {
          ...
          width:
            375 + parseInt(i.getLang("advanced.colorpicker_delta_width", 0)),
          height:
            250 + parseInt(i.getLang("advanced.colorpicker_delta_height", 0)),
      ...
      i.windowManager.open(
        {
          ...
          width: parseInt(
            i.getParam("theme_advanced_source_editor_width", 720)
          ),
          height: parseInt(
    ...
    _mceImage: function (j, k) {
      var i = this.editor;
      if (
        i.dom.getAttrib(i.selection.getNode(), "class").indexOf("mceItem") != -1
      ) {
        return;
      }
      i.windowManager.open(
