...
class FedenaPlugin
  ...
  def self.register=(plugin_details)
    unless AVAILABLE_MODULES.collect { |mod| mod[:name] }.include?(plugin_details[:name])
      ...
      unless plugin_details[:student_profile_more_menu].blank?
        ADDITIONAL_LINKS[:student_profile_more_menu] << plugin_details[:student_profile_more_menu]
      end
      unless plugin_details[:employee_profile_more_menu].blank?
        ADDITIONAL_LINKS[:employee_profile_more_menu] << plugin_details[:employee_profile_more_menu]
      end
      unless plugin_details[:online_exam_index_link].blank?
        ADDITIONAL_LINKS[:online_exam_index_link] << plugin_details[:online_exam_index_link]
      end
      unless plugin_details[:instant_fees_index_link].blank?
        ADDITIONAL_LINKS[:instant_fees_index_link] << plugin_details[:instant_fees_index_link]
      end
      unless plugin_details[:autosuggest_menuitems].blank?
        ADDITIONAL_LINKS[:autosuggest_menuitems] << plugin_details[:autosuggest_menuitems]
      end
      ...
      unless plugin_details[:auth_file].blank?
